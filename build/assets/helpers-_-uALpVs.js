...
const m = {
 ...
 _scrollToActive(e = !1, t = 500) {
  ...
  if (s) {
   const i = (a, c, u, h) => (
     (a /= h / 2),
     a < 1 ? (u / 2) * a * a + c : ((a -= 1), (-u / 2) * (a * (a - 2) - 1) + c)
    ),
    o = this.getLayoutMenu().querySelector('.menu-inner');
   if (
    (typeof s == 'string' && (s = document.querySelector(s)),
    typeof s != 'number' && (s = s.getBoundingClientRect().top + o.scrollTop),
    s < parseInt((o.clientHeight * 2) / 3, 10))
   )
    return;
   const r = o.scrollTop,
    d = s - r - parseInt(o.clientHeight / 2, 10),
    l = +new Date();
   if (e === !0) {
    const a = () => {
     const u = +new Date() - l,
      h = i(u, r, d, t);
     (o.scrollTop = h), u < t ? requestAnimationFrame(a) : (o.scrollTop = d);
    };
    a();
   } else o.scrollTop = d;
  }
 ...
 _findParent(e, t) {
  if (
   (e && e.tagName.toUpperCase() === 'BODY') ||
   e.tagName.toUpperCase() === 'HTML'
  )
   return null;
  for (
   e = e.parentNode;
   e && e.tagName.toUpperCase() !== 'BODY' && !e.classList.contains(t);
  ...
  if (!(typeof window > 'u'))
   if (document.createEvent) {
    ...
    typeof Event == 'function'
     ? (t = new Event(e))
     : ((t = document.createEvent('Event')), t.initEvent(e, !1, !0)),
     window.dispatchEvent(t);
   } else window.fireEvent(`on${e}`, document.createEventObject());
 },
 _triggerEvent(e) {
  this._triggerWindowEvent(`layout${e}`),
   this._listeners
    .filter((t) => t.event === e)
    .forEach((t) => t.callback.call(null));
 },
 ...
 _getNavbarHeight() {
  ...
  (t.id = null),
   ...
   Array.prototype.slice
    .call(t.querySelectorAll('.collapse.show'))
    .forEach((s) => this._removeClass('show', s)),
 ...
 _bindLayoutAnimationEndEvent(e, t) {
  ...
  (this._transitionCallback = (i) => {
   ...
   (this._transitionCallbackTimeout = setTimeout(() => {
    this._transitionCallback.call(this, { target: n });
   }, s));
 },
 ...
 _bindWindowResizeEvent() {
  ...
  (this._resizeCallback = () => {
   this._resizeTimeout && clearTimeout(this._resizeTimeout),
    (this._resizeTimeout = setTimeout(e, this.RESIZE_DELAY));
  }),
  ...
  this._menuMouseEnter ||
   ...
   this._windowTouchStart ||
    ((this._windowTouchStart = (t) => {
     (!t || !t.target || !this._findParent(t.target, '.layout-menu')) &&
      this._setMenuHoverState(!1);
    }),
 ...
 _unbindMenuMouseEvents() {
  ...
  this._menuMouseEnter &&
   ...
   this._menuMouseLeave &&
    (e && e.removeEventListener('mouseleave', this._menuMouseLeave, !1),
    (this._menuMouseLeave = null)),
   this._windowTouchStart &&
    (e && window.addEventListener('touchstart', this._windowTouchStart, !0),
 ...
 scrollPageTo(e, t = 500) {
  const n = (l, a, c, u) => (
    (l /= u / 2),
    l < 1 ? (c / 2) * l * l + a : ((l -= 1), (-c / 2) * (l * (l - 2) - 1) + a)
   ),
   s = document.scrollingElement;
  typeof e == 'string' && (e = document.querySelector(e)),
   typeof e != 'number' && (e = e.getBoundingClientRect().top + s.scrollTop);
  const i = s.scrollTop,
   o = e - i,
   r = +new Date(),
   d = () => {
    const a = +new Date() - r,
     c = n(a, i, o, t);
    (s.scrollTop = c), a < t ? requestAnimationFrame(d) : (s.scrollTop = e);
   };
  d();
 },
 ...
 setContentLayout(e = f('contentLayout')) {
  setTimeout(() => {
   const t = document.querySelector('.content-wrapper > div'),
    ...
    d = [].slice.call(document.querySelectorAll('.container-fluid')),
   ...
   document.querySelector('.content-wrapper > .menu-horizontal > div') &&
    ((c = !0),
    (u = document.querySelector('.content-wrapper > .menu-horizontal > div'))),
    e === 'compact'
     ? (d.some((h) => [t, r].includes(h)) &&
        (this._removeClass('container-fluid', [t, r]),
        this._addClass('container-xxl', [t, r])),
       o &&
        (this._removeClass('container-fluid', [o]),
        this._addClass('container-xxl', [o])),
       a &&
        d.some((h) => [n].includes(h)) &&
        (this._removeClass('container-fluid', [n]),
        this._addClass('container-xxl', [n])),
       c &&
        (this._removeClass('container-fluid', u),
        this._addClass('container-xxl', u),
        s &&
         (this._removeClass('container-fluid', s),
         this._addClass('container-xxl', s)),
        i &&
         (this._removeClass('container-fluid', i),
         this._addClass('container-xxl', i))))
     : (l.some((h) => [t, r].includes(h)) &&
        (this._removeClass('container-xxl', [t, r]),
        this._addClass('container-fluid', [t, r])),
       o &&
        (this._removeClass('container-xxl', [o]),
        this._addClass('container-fluid', [o])),
       a &&
        l.some((h) => [n].includes(h)) &&
        (this._removeClass('container-xxl', [n]),
        this._addClass('container-fluid', [n])),
       c &&
        (this._removeClass('container-xxl', u),
        this._addClass('container-fluid', u),
        s &&
         (this._removeClass('container-xxl', s),
         this._addClass('container-fluid', s)),
        i &&
 ...
 on(e = f('event'), t = f('callback')) {
  ...
  let [, ...s] = e.split('.');
  (s = s.join('.') || null),
   this._listeners.push({ event: n, namespace: s, callback: t });
 },
 off(e = f('event')) {
  const [t] = e.split('.');
  let [, ...n] = e.split('.');
  (n = n.join('.') || null),
   this._listeners
    .filter((s) => s.event === t && s.namespace === n)
    .forEach((s) => this._listeners.splice(this._listeners.indexOf(s), 1));
 },
 init() {
  this._initialized ||
   ((this._initialized = !0),
    ...
    this.off('resize._Helpers:redrawMenu'),
     ...
     typeof document.documentMode == 'number' &&
      ...
      this.on('resize._Helpers:ie10RepaintBody', () => {
       ...
       const { scrollTop: e } = document.documentElement;
  ...
  if (e != null && typeof t < 'u' && t !== null) {
   ...
   document.querySelectorAll('.speech-to-text i').forEach((i) => {
    ...
    i.addEventListener('click', () => {
     i.closest('.input-group').querySelector('.form-control').focus(),
      (n.onspeechstart = () => {
       o = !0;
      }),
      o === !1 && n.start(),
      ...
      (n.onresult = (r) => {
       i.closest('.input-group').querySelector('.form-control').value =
        r.results[0][0].transcript;
      }),
      (n.onspeechend = () => {
       (o = !1), n.stop();
      });
 ...
 initNavbarDropdownScrollbar() {
  const e = document.querySelectorAll('.navbar-dropdown .scrollable-container'),
   { PerfectScrollbar: t } = window;
  t !== void 0 &&
   typeof e < 'u' &&
   e !== null &&
   e.forEach((n) => {
    new t(n, { wheelPropagation: !1, suppressScrollX: !0 });
   });
 },
 ajaxCall(e) {
  return new Promise((t, n) => {
   ...
   s.open('GET', e),
    (s.onload = () =>
     s.status === 200 ? t(s.response) : n(Error(s.statusText))),
    (s.onerror = (i) => n(Error(`Network Error: ${i}`))),
    s.send();
  });
 },
 initSidebarToggle() {
    ...
    document.querySelectorAll(n).forEach((r) => {
     r.classList.toggle('show'),
      typeof s < 'u' &&
       ...
       i[0].addEventListener('click', (d) => {
        d.currentTarget.classList.remove('show'), r.classList.remove('show');
       }));
    });
   });
  });
