...
function $e(s, e) {
 s === void 0 && (s = {}),
  ...
  Object.keys(e).forEach((t) => {
   typeof s[t] > 'u'
    ? (s[t] = e[t])
    : Be(e[t]) && Be(s[t]) && Object.keys(e[t]).length > 0 && $e(s[t], e[t]);
  });
}
const Ve = {
 body: {},
    ...
    n !== void 0 &&
     ...
     (oe(s[c]) && oe(i[c])
      ? i[c].__swiper__
        ...
        : _(s[c], i[c])
      : !oe(s[c]) && oe(i[c])
        ? ((s[c] = {}), i[c].__swiper__ ? (s[c] = i[c]) : _(s[c], i[c]))
        : (s[c] = i[c]));
   }
  }
 }
...
function st(s) {
 ...
 return (
  !o &&
   ...
   o || (o = [0, 1, '13_0_0']),
   (h = !1)),
  p && !m && ((f.os = 'android'), (f.android = !0)),
  (o || d || l) && ((f.os = 'ios'), (f.ios = !0)),
  f
 );
}
function _e(s) {
 return s === void 0 && (s = {}), Te || (Te = st(s)), Te;
}
let Me;
function it() {
 const s = R(),
 ...
 const a = /(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)/i.test(
  ...
  r = i(),
  f = r || (a && e.ios);
 return { isSafari: t || r, needPerspectiveFix: t, need3dFix: f, isWebView: a };
}
function rt() {
 return Me || (Me = it()), Me;
}
    ...
    ((r = new ResizeObserver((l) => {
     f = a.requestAnimationFrame(() => {
      ...
      let h = d,
       v = m;
      l.forEach((g) => {
       let { contentBoxSize: u, contentRect: y, target: w } = g;
       (w && w !== e.el) ||
        ((h = y ? y.width : (u[0] || u).inlineSize),
        (v = y ? y.height : (u[0] || u).blockSize));
      }),
       (h !== d || v !== m) && c();
     });
    })),
  ...
  c = function (o, l) {
   ...
   const d = f.MutationObserver || f.WebkitMutationObserver,
    m = new d((h) => {
     ...
     f.requestAnimationFrame ? f.requestAnimationFrame(v) : f.setTimeout(v, 0);
    });
   m.observe(o, {
  ...
  return (
   ...
   (Array.isArray(e) ? e : e.split(' ')).forEach((n) => {
    s.eventsAnyListeners &&
     ...
     s.eventsAnyListeners.forEach((p) => {
      p.apply(i, [n, ...t]);
     }),
     s.eventsListeners &&
      s.eventsListeners[n] &&
      s.eventsListeners[n].forEach((p) => {
       p.apply(i, t);
      });
   }),
   s
...
function dt() {
 const s = this;
 function e(T, z) {
  return parseFloat(T.getPropertyValue(s.getDirectionLabel(z)) || 0);
 }
 const t = s.params,
  { wrapperEl: i, slidesEl: a, size: r, rtlTranslate: f, wrongRTL: c } = s,
  n = s.virtual && t.virtual.enabled,
  p = n ? s.virtual.slides.length : s.slides.length,
  o = Y(a, `.${s.params.slideClass}, swiper-slide`),
 ...
 typeof w == 'string' && w.indexOf('%') >= 0
  ...
  (s.virtualSize = -w),
  o.forEach((T) => {
   f ? (T.style.marginLeft = '') : (T.style.marginRight = ''),
    (T.style.marginBottom = ''),
    (T.style.marginTop = '');
  }),
  t.centeredSlides &&
   t.cssMode &&
   (de(i, '--swiper-centered-offset-before', ''),
 ...
 for (let T = 0; T < l; T += 1) {
  ...
  if (
   (o[T] && (z = o[T]),
   O && s.grid.updateSlide(T, z, o),
   !(o[T] && J(z, 'display') === 'none'))
  ) {
   if (t.slidesPerView === 'auto') {
    A && (o[T].style[s.getDirectionLabel('width')] = '');
    const x = getComputedStyle(z),
     b = z.style.transform,
     S = z.style.webkitTransform;
    if (
     (b && (z.style.transform = 'none'),
     S && (z.style.webkitTransform = 'none'),
     t.roundLengths)
    )
     M = s.isHorizontal() ? ze(z, 'width', !0) : ze(z, 'height', !0);
    else {
     const C = e(x, 'width'),
      k = e(x, 'padding-left'),
      D = e(x, 'padding-right'),
      L = e(x, 'margin-left'),
      $ = e(x, 'margin-right'),
      G = x.getPropertyValue('box-sizing');
     if (G && G === 'border-box') M = C + L + $;
     else {
      const { clientWidth: N, offsetWidth: F } = z;
      M = C + k + D + L + $ + (F - N);
     }
    }
    b && (z.style.transform = b),
     S && (z.style.webkitTransform = S),
     t.roundLengths && (M = Math.floor(M));
   } else
    (M = (r - (t.slidesPerView - 1) * w) / t.slidesPerView),
     t.roundLengths && (M = Math.floor(M)),
     o[T] && (o[T].style[s.getDirectionLabel('width')] = `${M}px`);
   o[T] && (o[T].swiperSlideSize = M),
    ...
    t.centeredSlides
     ? ((E = E + M / 2 + I / 2 + w),
       ...
       P % t.slidesPerGroup === 0 && d.push(E),
       m.push(E))
     : (t.roundLengths && (E = Math.floor(E)),
       (P - Math.min(s.params.slidesPerGroupSkip, P)) %
 ...
 if (
  ...
  f &&
   ...
   (i.style.width = `${s.virtualSize + w}px`),
  t.setWrapperSize &&
   (i.style[s.getDirectionLabel('width')] = `${s.virtualSize + w}px`),
  O && s.grid.updateWrapperSize(M, d),
  !t.centeredSlides)
 ) {
  const T = [];
 ...
 if (n && t.loop) {
  ...
  if (t.slidesPerGroup > 1) {
   const z = Math.ceil(
    ...
    x = T * t.slidesPerGroup;
   for (let b = 0; b < z; b += 1) d.push(d[d.length - 1] + x);
  }
 ...
 if ((d.length === 0 && (d = [0]), w !== 0)) {
  ...
  o.filter((z, x) => (!t.cssMode || t.loop ? !0 : x !== o.length - 1)).forEach(
   (z) => {
    z.style[T] = `${w}px`;
   },
  );
 ...
 if (
  ...
  de(i, '--swiper-centered-offset-before', `${-d[0]}px`),
   de(
    i,
  ...
  const T = -s.snapGrid[0],
   z = -s.slidesGrid[0];
  (s.snapGrid = s.snapGrid.map((x) => x + T)),
   (s.slidesGrid = s.slidesGrid.map((x) => x + z));
 }
 if (
  (l !== p && s.emit('slidesLengthChange'),
  d.length !== u &&
   (s.params.watchOverflow && s.checkOverflow(),
   s.emit('snapGridLengthChange')),
  m.length !== y && s.emit('slidesGridLengthChange'),
  ...
  const T = `${t.containerModifierClass}backface-hidden`,
   z = s.el.classList.contains(T);
  l <= t.maxBackfaceHiddenSlides
   ? z || s.el.classList.add(T)
   : z && s.el.classList.remove(T);
 }
}
function ct(s) {
 const e = this,
...
function mt() {
 const s = this,
  ...
  c = (l) => Y(i, `.${t.slideClass}${l}, swiper-slide${l}`)[0];
 e.forEach((l) => {
 ...
 n &&
  ...
  f
   ...
   : ((o = et(n, `.${t.slideClass}, swiper-slide`)[0]),
     t.loop && !o && (o = e[0]),
     o && o.classList.add(t.slideNextClass),
     (p = Je(n, `.${t.slideClass}, swiper-slide`)[0]),
...
const he = (s, e) => {
 ...
 Ae = (s) => {
  ...
  if (s.params.grid && s.params.grid.rows > 1) {
   const f = a,
    c = [f - e];
   c.push(...Array.from({ length: e }).map((n, p) => f + i + p)),
    s.slides.forEach((n, p) => {
     c.includes(n.column) && Ce(s, p);
    });
   return;
  }
  const r = a + i - 1;
...
function gt(s) {
 ...
 if ((typeof n > 'u' && (n = ht(e)), i.indexOf(t) >= 0)) p = i.indexOf(t);
 else {
  const m = Math.min(a.slidesPerGroupSkip, n);
  p = m + Math.floor((n - m) / a.slidesPerGroup);
 }
 if ((p >= i.length && (p = i.length - 1), n === r && !e.params.loop)) {
  p !== c && ((e.snapIndex = p), e.emit('snapIndexChange'));
  return;
 }
 ...
 t.isHorizontal() ? (c = i ? -s : s) : (n = s),
  ...
  a.cssMode
   ...
   : a.virtualTranslate ||
     ...
     (r.style.transform = `translate3d(${c}px, ${n}px, ${p}px)`));
 let o;
 ...
 if (
  ...
  if (e === 0) c[l ? 'scrollLeft' : 'scrollTop'] = -o;
   ...
   if (!r.support.smoothScroll)
    return Fe({ swiper: r, targetPosition: -o, side: l ? 'left' : 'top' }), !0;
...
function Ct(s, e) {
 ...
 i.cssMode ||
  ...
  qe({ swiper: t, runCallbacks: s, direction: e, step: 'Start' }));
}
function Pt(s, e) {
 s === void 0 && (s = !0);
 const t = this,
 ...
 (t.animating = !1),
  !i.cssMode &&
   (t.setTransition(0),
   qe({ swiper: t, runCallbacks: s, direction: e, step: 'End' }));
}
var Lt = { setTransition: Mt, transitionStart: Ct, transitionEnd: Pt };
function It(s, e, t, i, a) {
 s === void 0 && (s = 0),
  e === void 0 && (e = this.params.speed),
  t === void 0 && (t = !0),
 ...
 if (
  ...
  return !1;
 const v = Math.min(r.params.slidesPerGroupSkip, f);
 let g = v + Math.floor((f - v) / r.params.slidesPerGroup);
 g >= n.length && (g = n.length - 1);
 const u = -n[g];
 if (c.normalizeSlideIndex)
  for (let w = 0; w < p.length; w += 1) {
   const E = -Math.floor(u * 100),
    I = Math.floor(p[w] * 100),
    P = Math.floor(p[w + 1] * 100);
   typeof p[w + 1] < 'u'
    ? E >= I && E < P - (P - I) / 2
      ? (f = w)
      : E >= I && E < P && (f = w + 1)
 ...
 if (
  ...
  return !1;
 f !== (o || 0) && t && r.emit('beforeSlideChangeStart'), r.updateProgress(u);
 let y;
 if (
  (f > l ? (y = 'next') : f < l ? (y = 'prev') : (y = 'reset'),
  (d && -u === r.translate) || (!d && u === r.translate))
 )
  return (
   r.updateActiveIndex(f),
   c.autoHeight && r.updateAutoHeight(),
   r.updateSlidesClasses(),
   c.effect !== 'slide' && r.setTranslate(u),
 ...
 if (c.cssMode) {
  ...
  if (e === 0) {
   ...
   if (!r.support.smoothScroll)
    return Fe({ swiper: r, targetPosition: E, side: w ? 'left' : 'top' }), !0;
...
function $t(s, e, t) {
 ...
 if (a.loop) {
  if (p && !o && a.loopPreventsSliding) return !1;
...
function Dt(s, e, t) {
 s === void 0 && (s = this.params.speed), e === void 0 && (e = !0);
 const i = this;
 if (!i.destroyed) return i.slideTo(i.activeIndex, s, e, t);
}
function Ot(s, e, t, i) {
 ...
 if (n >= a.snapGrid[c]) {
  const p = a.snapGrid[c],
   o = a.snapGrid[c + 1];
  n - p > (o - p) * i && (r += a.params.slidesPerGroup);
 } else {
  const p = a.snapGrid[c - 1],
   o = a.snapGrid[c];
  n - p <= (o - p) * i && (r -= a.params.slidesPerGroup);
 }
 return (
  ...
  (r = parseInt(s.clickedSlide.getAttribute('data-swiper-slide-index'), 10)),
   e.centeredSlides
    ? a < s.loopedSlides - i / 2 || a > s.slides.length - s.loopedSlides + i / 2
      ? (s.loopFix(),
        (a = s.getSlideIndex(Y(t, `${f}[data-swiper-slide-index="${r}"]`)[0])),
        se(() => {
         s.slideTo(a);
        }))
      : s.slideTo(a)
...
function Ht(s) {
 ...
 const a = () => {
   Y(i, `.${t.slideClass}, swiper-slide`).forEach((l, d) => {
 ...
 if (c) {
  if (t.loopAddBlankSlides) {
   ...
   p(o), e.recalcSlides(), e.updateSlides();
  } else
   ge(
...
function Bt(s) {
 ...
 typeof r > 'u'
  ? (r = n.getSlideIndex(
     p.filter((b) => b.classList.contains(m.slideActiveClass))[0],
    ))
  : (I = r);
 const P = i === 'next' || !i,
  O = i === 'prev' || !i;
 let M = 0,
  A = 0;
 const T = y ? Math.ceil(p.length / m.grid.rows) : p.length,
  x = (y ? p[r].column : r) + (h && typeof a > 'u' ? -v / 2 + 0.5 : 0);
 if (x < u) {
  M = Math.max(u - x, g);
  for (let b = 0; b < u - x; b += 1) {
   const S = b - Math.floor(b / T) * T;
   if (y) {
    const C = T - S - 1;
    for (let k = p.length - 1; k >= 0; k -= 1) p[k].column === C && w.push(k);
   } else w.push(T - S - 1);
  }
 } else if (x + v > T - u) {
  A = Math.max(x - (T - u * 2), g);
  for (let b = 0; b < A; b += 1) {
   const S = b - Math.floor(b / T) * T;
   y
    ? p.forEach((C, k) => {
 ...
 if (
  ...
  O &&
   w.forEach((b) => {
    (p[b].swiperLoopMoveDOM = !0),
     d.prepend(p[b]),
     (p[b].swiperLoopMoveDOM = !1);
   }),
  P &&
   E.forEach((b) => {
    (p[b].swiperLoopMoveDOM = !0),
  ...
  m.slidesPerView === 'auto'
   ? n.updateSlides()
   : y &&
     ((w.length > 0 && O) || (E.length > 0 && P)) &&
     n.slides.forEach((b, S) => {
      n.grid.updateSlide(S, b, n.slides);
     }),
  ...
  if (w.length > 0 && O) {
   if (typeof e > 'u') {
    const b = n.slidesGrid[I],
     C = n.slidesGrid[I + M] - b;
    c
     ? n.setTranslate(n.translate - C)
     : (n.slideTo(I + M, 0, !1, !0),
       a &&
        ((n.touchEventsData.startTranslate =
         n.touchEventsData.startTranslate - C),
        (n.touchEventsData.currentTranslate =
         n.touchEventsData.currentTranslate - C)));
   } else if (a) {
    const b = y ? w.length / m.grid.rows : w.length;
   ...
   if (typeof e > 'u') {
    ...
    const b = y ? E.length / m.grid.rows : E.length;
    n.slideTo(n.activeIndex - b, 0, !1, !0);
   }
 }
 if (
  ...
  Array.isArray(n.controller.control)
   ? n.controller.control.forEach((S) => {
      !S.destroyed &&
       S.params.loop &&
       S.loopFix({
        ...b,
        slideTo: S.params.slidesPerView === m.slidesPerView ? t : !1,
       });
     })
   : n.controller.control instanceof n.constructor &&
     ...
     n.controller.control.loopFix({
      ...b,
      slideTo:
       n.controller.control.params.slidesPerView === m.slidesPerView ? t : !1,
     });
 }
 n.emit('loopFix');
}
...
var Nt = { loopCreate: Ht, loopFix: Bt, loopDestroy: Xt };
function Rt(s) {
 const e = this;
 if (
...
var Vt = { setGrabCursor: Rt, unsetGrabCursor: Yt };
function Ft(s, e) {
 e === void 0 && (e = this);
 function t(i) {
...
function Wt(s) {
 ...
 if (i.type === 'touchstart') {
  Xe(e, i, i.targetTouches[0].pageX);
  return;
 }
...
function _t(s) {
 ...
 if (n.type === 'touchmove') {
  if (
   ((p = [...n.changedTouches].filter((P) => P.identifier === i.touchId)[0]),
 ...
 if (typeof i.isScrolling > 'u') {
  ...
  (t.isVertical() && r.currentX === r.startX)
   ? (i.isScrolling = !1)
   : d * d + m * m >= 25 &&
     ((P = (Math.atan2(Math.abs(m), Math.abs(d)) * 180) / Math.PI),
 ...
 const u = t.params.loop && !a.cssMode,
  y =
   (t.touchesDirection === 'next' && t.allowSlideNext) ||
   (t.touchesDirection === 'prev' && t.allowSlidePrev);
 if (!i.isMoved) {
  if (
   (u && y && t.loopFix({ direction: t.swipeDirection }),
     ...
     i.currentTranslate > t.minTranslate() &&
      ...
      a.resistance &&
       (i.currentTranslate =
        t.minTranslate() -
        1 +
        (-t.minTranslate() + i.startTranslate + h) ** I)))
   : h < 0 &&
     (u &&
      y &&
      !w &&
     ...
     i.currentTranslate < t.maxTranslate() &&
      ...
      a.resistance &&
       (i.currentTranslate =
        t.maxTranslate() +
        1 -
        (t.maxTranslate() - i.startTranslate - h) ** I))),
...
function qt(s) {
 ...
 if (i.type === 'touchend' || i.type === 'touchcancel') {
  if (
   ((a = [...i.changedTouches].filter((I) => I.identifier === t.touchId)[0]),
 ...
 if (e.allowClick) {
  const I = i.path || (i.composedPath && i.composedPath());
  e.updateClickedSlide((I && I[0]) || i.target, I),
   e.emit('tap click', i),
 ...
 for (
  let I = 0;
  I < p.length;
  I += I < f.slidesPerGroupSkip ? 1 : f.slidesPerGroup
 ) {
  const P = I < f.slidesPerGroupSkip - 1 ? 1 : f.slidesPerGroup;
  typeof p[I + P] < 'u'
   ? (h || (m >= p[I] && m < p[I + P])) && ((v = I), (g = p[I + P] - p[I]))
   : (h || m >= p[I]) && ((v = I), (g = p[p.length - 1] - p[p.length - 2]));
 }
 let u = null,
  y = null;
 f.rewind &&
  (e.isBeginning
   ...
   : e.isEnd && (u = 0));
 const w = (m - p[v]) / g,
  E = v < f.slidesPerGroupSkip - 1 ? 1 : f.slidesPerGroup;
 if (d > f.longSwipesMs) {
  if (!f.longSwipes) {
  ...
  e.swipeDirection === 'next' &&
   (w >= f.longSwipesRatio
    ? e.slideTo(f.rewind && e.isEnd ? u : v + E)
    : e.slideTo(v)),
   e.swipeDirection === 'prev' &&
    (w > 1 - f.longSwipesRatio
     ? e.slideTo(v + E)
     : y !== null && w < 0 && Math.abs(w) > f.longSwipesRatio
       ? e.slideTo(y)
       : e.slideTo(v));
 } else {
  if (!f.shortSwipes) {
  ...
  (i.target === e.navigation.nextEl || i.target === e.navigation.prevEl)
   ? i.target === e.navigation.nextEl
     ? e.slideTo(v + E)
     : e.slideTo(v)
   : (e.swipeDirection === 'next' && e.slideTo(u !== null ? u : v + E),
...
const je = (s, e) => {
 ...
 t[n]('touchstart', s.onDocumentTouchStart, { passive: !1, capture: c }),
  ...
  i.updateOnWindowResize
   ...
   : s[p]('observerUpdate', Ne, !0),
  a[n]('load', s.onLoad, { capture: !0 });
};
function Qt() {
 const s = this,
  { params: e } = s;
 (s.onTouchStart = Wt.bind(s)),
  (s.onTouchMove = _t.bind(s)),
  (s.onTouchEnd = qt.bind(s)),
  (s.onDocumentTouchStart = Zt.bind(s)),
  e.cssMode && (s.onScroll = Ut.bind(s)),
  (s.onClick = jt.bind(s)),
  (s.onLoad = Kt.bind(s)),
  je(s, 'on');
}
...
var es = { attachEvents: Qt, detachEvents: Jt };
const Re = (s, e) => s.grid && e.grid && e.grid.rows > 1;
function ts() {
 const s = this,
 ...
 p && !o
  ...
  ['navigation', 'pagination', 'scrollbar'].forEach((u) => {
   if (typeof n[u] > 'u') return;
   const y = i[u] && i[u].enabled,
    w = n[u] && n[u].enabled;
   y && !w && s[u].disable(), !y && w && s[u].enable();
  });
...
var is = { setBreakpoint: ts, getBreakpoint: ss };
function rs(s, e) {
 const t = [];
 return (
  s.forEach((i) => {
...
var ls = { addClasses: as, removeClasses: ns };
function os() {
 const s = this,
...
class W {
 constructor() {
  ...
  Object.prototype.toString.call(a[0]).slice(8, -1) === 'Object'
   ...
   e && !t.el && (t.el = e);
  const f = V();
  if (t.el && typeof t.el == 'string' && f.querySelectorAll(t.el).length > 1) {
   const o = [];
   return (
 ...
 getSlideIndex(e) {
  const { slidesEl: t, params: i } = this,
   a = Y(t, `.${i.slideClass}, swiper-slide`),
   r = fe(a[0]);
  return fe(e) - r;
 }
 ...
 getSlideClasses(e) {
  ...
  return t.destroyed
   ...
   : e.className
      ...
      .filter(
       (i) =>
        i.indexOf('swiper-slide') === 0 || i.indexOf(t.params.slideClass) === 0,
      )
      .join(' ');
 }
 emitSlidesClasses() {
 ...
 slidesPerViewDynamic(e, t) {
  ...
  if (a.centeredSlides) {
   let l = r[p] ? Math.ceil(r[p].swiperSlideSize) : 0,
    d;
   for (let m = p + 1; m < r.length; m += 1)
    r[m] &&
     !d &&
   ...
   for (let l = p + 1; l < r.length; l += 1)
    (t ? f[l] + c[l] - f[p] < n : f[l] - f[p] < n) && (o += 1);
  else for (let l = p - 1; l >= 0; l -= 1) f[p] - f[l] < n && (o += 1);
  return o;
 }
 update() {
  const e = this;
 ...
 mount(e) {
  ...
  return (
   !f &&
    ...
    ((f = q('div', t.params.wrapperClass)),
    i.append(f),
    Y(i, `.${t.params.slideClass}`).forEach((c) => {
     f.append(c);
    })),
   Object.assign(t, {
    el: i,
    ...
    wrongRTL: J(f, 'display') === '-webkit-box',
   }),
   !0
  );
 }
 ...
 static use(e) {
  return Array.isArray(e)
   ? (e.forEach((t) => W.installModule(t)), W)
   : (W.installModule(e), W);
 }
}
Object.keys(Pe).forEach((s) => {
 Object.keys(Pe[s]).forEach((e) => {
  W.prototype[e] = Pe[s][e];
 });
});
W.use([at, nt]);
function fs(s) {
 ...
 function n(h, v) {
  ...
  return (
   g.renderSlide
    ? ((u = g.renderSlide.call(e, h, v)),
      typeof u == 'string' && ((c.innerHTML = u), (u = c.children[0])))
    : e.isElement
      ? (u = q('swiper-slide'))
      : (u = q('div', e.params.slideClass)),
   u.setAttribute('data-swiper-slide-index', v),
   g.renderSlide || (u.innerHTML = h),
   g.cache && (e.virtual.cache[v] = u),
   u
  );
 }
 function p(h) {
  const {
   ...
   { addSlidesBefore: w, addSlidesAfter: E } = e.params.virtual,
   { from: I, to: P, slides: O, slidesGrid: M, offset: A } = e.virtual;
  ...
  let x, b;
  u
   ? ((x = Math.floor(v / 2) + g + E), (b = Math.floor(v / 2) + g + w))
   : ((x = v + (g - 1) + E), (b = (y ? v : g) + w));
  let S = T - b,
   C = T + x;
  y || ((S = Math.max(S, 0)), (C = Math.min(C, O.length - 1)));
  let k = (e.slidesGrid[S] || 0) - (e.slidesGrid[0] || 0);
  y && T >= b
   ? ((S -= b), u || (k += e.slidesGrid[0]))
   : y && T < b && ((S = -b), u && (k += e.slidesGrid[0])),
  ...
  if (I === S && P === C && !h) {
   e.slidesGrid !== M &&
    k !== A &&
    e.slides.forEach((H) => {
     H.style[z] = `${k - Math.abs(e.cssOverflowAdjustment())}px`;
    }),
    e.updateProgress(),
    a('virtualUpdate');
   return;
  ...
  if (e.params.virtual.renderExternal) {
   e.params.virtual.renderExternal.call(e, {
    ...
    slides: (function () {
     const X = [];
     for (let j = S; j <= C; j += 1) X.push(O[j]);
     return X;
    })(),
   }),
    e.params.virtual.renderExternalUpdate ? D() : a('virtualUpdate');
   return;
  }
  const L = [],
   $ = [],
   G = (H) => {
    let X = H;
    return H < 0 ? (X = O.length + H) : X >= O.length && (X = X - O.length), X;
   };
  if (h)
   e.slides
  ...
  const N = y ? -O.length : 0,
   F = y ? O.length * 2 : O.length;
  for (let H = N; H < F; H += 1)
   if (H >= S && H <= C) {
    const X = G(H);
    typeof P > 'u' || h ? $.push(X) : (H > P && $.push(X), H < I && L.push(X));
   }
  if (
   ($.forEach((H) => {
    e.slidesEl.append(n(O[H], H));
   }),
   y)
  )
   for (let H = L.length - 1; H >= 0; H -= 1) {
    const X = L[H];
    e.slidesEl.prepend(n(O[X], X));
   }
  else
   L.sort((H, X) => X - H),
    L.forEach((H) => {
     e.slidesEl.prepend(n(O[H], H));
    });
  Y(e.slidesEl, '.swiper-slide, swiper-slide').forEach((H) => {
   H.style[z] = `${k - Math.abs(e.cssOverflowAdjustment())}px`;
  }),
   D();
 }
 ...
 function l(h) {
  ...
  let g = v + 1,
   u = 1;
  if (Array.isArray(h)) {
   for (let y = 0; y < h.length; y += 1) h[y] && e.virtual.slides.unshift(h[y]);
   (g = v + h.length), (u = h.length);
  ...
  if (e.params.virtual.cache) {
   ...
   Object.keys(y).forEach((E) => {
    const I = y[E],
     P = I.getAttribute('data-swiper-slide-index');
    P && I.setAttribute('data-swiper-slide-index', parseInt(P, 10) + u),
     (w[parseInt(E, 10) + u] = I);
   }),
    (e.virtual.cache = w);
  }
  ...
  if (Array.isArray(h))
   for (let g = h.length - 1; g >= 0; g -= 1)
    e.params.virtual.cache &&
     (delete e.virtual.cache[h[g]],
     Object.keys(e.virtual.cache).forEach((u) => {
      u > h &&
       ((e.virtual.cache[u - 1] = e.virtual.cache[u]),
       e.virtual.cache[u - 1].setAttribute('data-swiper-slide-index', u - 1),
       delete e.virtual.cache[u]);
     })),
     e.virtual.slides.splice(h[g], 1),
     h[g] < v && (v -= 1),
     (v = Math.max(v, 0));
  else
   e.params.virtual.cache &&
 ...
 i('beforeInit', () => {
  ...
  if (typeof e.passedParams.virtual.slides > 'u') {
   ...
   v &&
    ...
    (h = !0),
    v.forEach((g, u) => {
     g.setAttribute('data-swiper-slide-index', u),
      (e.virtual.cache[u] = g),
      g.remove();
    }));
  }
...
function us(s) {
 ...
 function c(o) {
  ...
  if (
   (!e.allowSlideNext &&
    ((e.isHorizontal() && y) || (e.isVertical() && E) || g)) ||
   (!e.allowSlidePrev &&
    ((e.isHorizontal() && u) || (e.isVertical() && w) || v))
  )
   return !1;
  if (
   ...
   if (e.params.keyboard.onlyInViewport && (v || g || u || y || w || E)) {
    let I = !1;
    if (
     te(e.el, `.${e.params.slideClass}, swiper-slide`).length > 0 &&
     te(e.el, `.${e.params.slideActiveClass}`).length === 0
    )
     return;
    const P = e.el,
     O = P.clientWidth,
   ...
   e.isHorizontal()
    ? ((v || g || u || y) &&
      ...
      (((g || y) && !l) || ((v || u) && l)) && e.slideNext(),
      (((v || u) && !l) || ((g || y) && l)) && e.slidePrev())
    : ((v || g || w || E) &&
       (d.preventDefault ? d.preventDefault() : (d.returnValue = !1)),
      (g || E) && e.slideNext(),
      (v || w) && e.slidePrev()),
...
function ps(s) {
 ...
 function o(w) {
  let O = 0,
   ...
   T = 0;
  return (
   'detail' in w && (M = w.detail),
   'wheelDelta' in w && (M = -w.wheelDelta / 120),
   'wheelDeltaY' in w && (M = -w.wheelDeltaY / 120),
   'wheelDeltaX' in w && (O = -w.wheelDeltaX / 120),
   'axis' in w && w.axis === w.HORIZONTAL_AXIS && ((O = M), (M = 0)),
   (A = O * 10),
   (T = M * 10),
   'deltaY' in w && (T = w.deltaY),
   'deltaX' in w && (A = w.deltaX),
   w.shiftKey && !A && ((A = T), (T = 0)),
   (A || T) &&
    w.deltaMode &&
    (w.deltaMode === 1 ? ((A *= 40), (T *= 40)) : ((A *= 800), (T *= 800))),
   A && !O && (O = A < 1 ? -1 : 1),
   T && !M && (M = T < 1 ? -1 : 1),
   { spinX: O, spinY: M, pixelX: A, pixelY: T }
  );
 }
 function l() {
 ...
 function m(w) {
  return (e.params.mousewheel.thresholdDelta &&
   ...
   (e.params.mousewheel.thresholdTime &&
    U() - c < e.params.mousewheel.thresholdTime)
   ? !1
   : w.delta >= 6 && U() - c < 60
     ? !0
     : (w.direction < 0
        ? (!e.isEnd || e.params.loop) &&
          !e.animating &&
          (e.slideNext(), a('scroll', w.raw))
        : (!e.isBeginning || e.params.loop) &&
          !e.animating &&
          (e.slidePrev(), a('scroll', w.raw)),
 ...
 function v(w) {
  ...
  const M = O && O.contains(E.target);
  if (!e.mouseEntered && !M && !P.releaseOnEdges) return !0;
  ...
  if (P.forceToAxis)
   if (e.isHorizontal())
    if (Math.abs(z.pixelX) > Math.abs(z.pixelY)) A = -z.pixelX * T;
    else return !0;
   else if (Math.abs(z.pixelY) > Math.abs(z.pixelX)) A = -z.pixelY;
   else return !0;
  else A = Math.abs(z.pixelX) > Math.abs(z.pixelY) ? -z.pixelX * T : -z.pixelY;
  if (A === 0) return !0;
  P.invert && (A = -A);
  let x = e.getTranslate() + A * P.sensitivity;
  if (
   ...
   I && e.params.nested && E.stopPropagation(),
   !e.params.freeMode || !e.params.freeMode.enabled)
  ) {
   const b = { time: U(), delta: Math.abs(A), direction: Math.sign(A), raw: w };
   p.length >= 2 && p.shift();
   ...
   const b = { time: U(), delta: Math.abs(A), direction: Math.sign(A) },
    S =
     n &&
     b.time < n.time + 500 &&
     b.delta <= n.delta &&
   ...
   if (!S) {
    n = void 0;
    let C = e.getTranslate() + A * P.sensitivity;
    ...
    if (
     ...
     e.updateActiveIndex(),
     e.updateSlidesClasses(),
     ((!k && e.isBeginning) || (!D && e.isEnd)) && e.updateSlidesClasses(),
     e.params.loop &&
      e.loopFix({
     ...
     const L = p.length ? p[p.length - 1] : void 0,
      $ = p[0];
     if ((p.push(b), L && (b.delta > L.delta || b.direction !== L.direction)))
      p.splice(0);
     else if (
      p.length >= 15 &&
      b.time - $.time < 500 &&
     ...
     f ||
      (f = se(() => {
       (n = b), p.splice(0), e.slideToClosest(e.params.speed, !0, void 0, 0.5);
      }, 500));
    }
    if (
     (S || a('scroll', E),
     e.params.autoplay &&
      ...
      e.autoplay.stop(),
     P.releaseOnEdges && (C === e.minTranslate() || C === e.maxTranslate()))
    )
     return !0;
   }
  }
 ...
 i('init', () => {
  ...
  i('destroy', () => {
   e.params.cssMode && u(), e.mousewheel.enabled && y();
  }),
  Object.assign(e.mousewheel, { enable: u, disable: y });
}
function De(s, e, t, i) {
 return (
  s.params.createElements &&
   Object.keys(i).forEach((a) => {
    if (!t[a] && t.auto === !0) {
     let r = Y(s.el, `.${i[a]}`)[0];
     r || ((r = q('div', i[a])), (r.className = i[a]), s.el.append(r)),
      (t[a] = r),
      (e[a] = r);
    }
   }),
...
function ms(s) {
 ...
 function f(h, v) {
  ...
  (h = B(h)),
   h.forEach((u) => {
    u &&
     (u.classList[v ? 'add' : 'remove'](...g.disabledClass.split(' ')),
     u.tagName === 'BUTTON' && (u.disabled = v),
     e.params.watchOverflow &&
      e.enabled &&
      u.classList[e.isLocked ? 'add' : 'remove'](g.lockClass));
   });
 }
 function c() {
  ...
  f(v, e.isBeginning && !e.params.rewind), f(h, e.isEnd && !e.params.rewind);
 }
 function n(h) {
  h.preventDefault(),
 ...
 function o() {
  ...
  let v = r(h.nextEl),
   g = r(h.prevEl);
  Object.assign(e.navigation, { nextEl: v, prevEl: g }), (v = B(v)), (g = B(g));
  const u = (y, w) => {
   y && y.addEventListener('click', w === 'next' ? p : n),
    !e.enabled && y && y.classList.add(...h.lockClass.split(' '));
  };
  v.forEach((y) => u(y, 'next')), g.forEach((y) => u(y, 'prev'));
 }
 function l() {
  let { nextEl: h, prevEl: v } = e.navigation;
  (h = B(h)), (v = B(v));
  const g = (u, y) => {
   u.removeEventListener('click', y === 'next' ? p : n),
    u.classList.remove(...e.params.navigation.disabledClass.split(' '));
  };
  h.forEach((u) => g(u, 'next')), v.forEach((u) => g(u, 'prev'));
 }
 i('init', () => {
  ...
  i('click', (h, v) => {
   let { nextEl: g, prevEl: u } = e.navigation;
   (g = B(g)), (u = B(u));
   const y = v.target;
   if (e.params.navigation.hideOnClick && !u.includes(y) && !g.includes(y)) {
    if (
     e.pagination &&
     e.params.pagination &&
    ...
    g.length
     ...
     : u.length &&
       (w = u[0].classList.contains(e.params.navigation.hiddenClass)),
     a(w === !0 ? 'navigationShow' : 'navigationHide'),
     [...g, ...u]
      .filter((E) => !!E)
...
function hs(s) {
 ...
 function p(u, y) {
  ...
  u &&
   ((u = u[`${y === 'prev' ? 'previous' : 'next'}ElementSibling`]),
   u &&
    (u.classList.add(`${w}-${y}`),
    (u = u[`${y === 'prev' ? 'previous' : 'next'}ElementSibling`]),
    u && u.classList.add(`${w}-${y}-${y}`)));
 }
 function o(u) {
  const y = u.target.closest(Z(e.params.pagination.bulletClass));
  if (!y) return;
  u.preventDefault();
  const w = fe(y) * e.params.slidesPerGroup;
  if (e.params.loop) {
 ...
 function l() {
  ...
  if (
   ...
   if (
    (y.dynamicBullets &&
     ((f = ze(M[0], e.isHorizontal() ? 'width' : 'height', !0)),
     w.forEach((x) => {
      x.style[e.isHorizontal() ? 'width' : 'height'] =
       `${f * (y.dynamicMainBullets + 4)}px`;
     }),
     y.dynamicMainBullets > 1 &&
      I !== void 0 &&
      ((c += E - (I || 0)),
      c > y.dynamicMainBullets - 1
       ? (c = y.dynamicMainBullets - 1)
       : c < 0 && (c = 0)),
     (A = Math.max(E - c, 0)),
     (T = A + (Math.min(M.length, y.dynamicMainBullets) - 1)),
     (z = (T + A) / 2)),
    M.forEach((x) => {
     const b = [
      ...['', '-next', '-next-next', '-prev', '-prev-prev', '-main'].map(
       (S) => `${y.bulletActiveClass}${S}`,
      ),
     ]
    ...
    if (
     ...
     e.isElement &&
      M.forEach((b, S) => {
       b.setAttribute('part', S === E ? 'bullet-active' : 'bullet');
      }),
     y.dynamicBullets)
    ) {
     const b = M[A],
      S = M[T];
     for (let C = A; C <= T; C += 1)
      M[C] && M[C].classList.add(...`${y.bulletActiveClass}-main`.split(' '));
     p(b, 'prev'), p(S, 'next');
    }
   }
   if (y.dynamicBullets) {
    const x = Math.min(M.length, y.dynamicMainBullets + 4),
     b = (f * x - f) / 2 - z * f,
     S = u ? 'right' : 'left';
    M.forEach((C) => {
     C.style[e.isHorizontal() ? S : 'top'] = `${b}px`;
    });
   }
  }
  w.forEach((M, A) => {
   if (
    (y.type === 'fraction' &&
     (M.querySelectorAll(Z(y.currentClass)).forEach((T) => {
      T.textContent = y.formatFractionCurrent(E + 1);
     }),
     M.querySelectorAll(Z(y.totalClass)).forEach((T) => {
      T.textContent = y.formatFractionTotal(O);
     })),
    y.type === 'progressbar')
   ) {
    let T;
    ...
    let x = 1,
     b = 1;
    T === 'horizontal' ? (x = z) : (b = z),
     M.querySelectorAll(Z(y.progressbarFillClass)).forEach((S) => {
      (S.style.transform = `translate3d(0,0,0) scaleX(${x}) scaleY(${b})`),
       (S.style.transitionDuration = `${e.params.speed}ms`);
     });
   }
   y.type === 'custom' && y.renderCustom
    ? ((M.innerHTML = y.renderCustom(e, E + 1, O)),
      A === 0 && a('paginationRender', M))
    : (A === 0 && a('paginationRender', M), a('paginationUpdate', M)),
    e.params.watchOverflow &&
     e.enabled &&
     M.classList[e.isLocked ? 'add' : 'remove'](y.lockClass);
  });
 }
 function d() {
  ...
  if (u.type === 'bullets') {
   let I = e.params.loop
    ...
    : e.snapGrid.length;
   e.params.freeMode && e.params.freeMode.enabled && I > y && (I = y);
   for (let P = 0; P < I; P += 1)
    u.renderBullet
     ? (E += u.renderBullet.call(e, P, u.bulletClass))
     : (E += `<${u.bulletElement} ${e.isElement ? 'part="bullet"' : ''} class="${u.bulletClass}"></${u.bulletElement}>`);
  }
  u.type === 'fraction' &&
   (u.renderFraction
    ? (E = u.renderFraction.call(e, u.currentClass, u.totalClass))
    : (E = `<span class="${u.currentClass}"></span> / <span class="${u.totalClass}"></span>`)),
   u.type === 'progressbar' &&
    (u.renderProgressbar
     ? (E = u.renderProgressbar.call(e, u.progressbarFillClass))
     : (E = `<span class="${u.progressbarFillClass}"></span>`)),
   (e.pagination.bullets = []),
   w.forEach((I) => {
    u.type !== 'custom' && (I.innerHTML = E || ''),
     u.type === 'bullets' &&
      e.pagination.bullets.push(...I.querySelectorAll(Z(u.bulletClass)));
   }),
   u.type !== 'custom' && a('paginationRender', w[0]);
 }
 function m() {
  ...
  if (!u.el) return;
  let y;
  typeof u.el == 'string' && e.isElement && (y = e.el.querySelector(u.el)),
   !y && typeof u.el == 'string' && (y = [...document.querySelectorAll(u.el)]),
   y || (y = u.el),
   !(!y || y.length === 0) &&
    (e.params.uniqueNavElements &&
     typeof u.el == 'string' &&
     Array.isArray(y) &&
     y.length > 1 &&
     ((y = [...e.el.querySelectorAll(u.el)]),
     y.length > 1 && (y = y.filter((w) => te(w, '.swiper')[0] === e.el)[0])),
    ...
    y.forEach((w) => {
     u.type === 'bullets' &&
      ...
      w.classList.add(e.isHorizontal() ? u.horizontalClass : u.verticalClass),
      u.type === 'bullets' &&
       u.dynamicBullets &&
      ...
      u.clickable && w.addEventListener('click', o),
      e.enabled || w.classList.add(u.lockClass);
    }));
 }
 function h() {
  const u = e.params.pagination;
  ...
  y &&
   ...
   y.forEach((w) => {
    w.classList.remove(u.hiddenClass),
     ...
     w.classList.remove(e.isHorizontal() ? u.horizontalClass : u.verticalClass),
     u.clickable &&
      (w.classList.remove(...(u.clickableClass || '').split(' ')),
      w.removeEventListener('click', o));
   })),
   e.pagination.bullets &&
 ...
 i('changeDirection', () => {
  ...
  i('init', () => {
   e.params.pagination.enabled === !1 ? g() : (m(), d(), l());
  }),
  i('activeIndexChange', () => {
  ...
  i('enable disable', () => {
   ...
   u &&
    ((u = B(u)),
    u.forEach((y) =>
     y.classList[e.enabled ? 'remove' : 'add'](e.params.pagination.lockClass),
    ));
  }),
  i('lock unlock', () => {
   l();
...
function gs(s) {
 ...
 function m() {
  ...
  b
   ...
   : $ < 0
     ...
     : $ + o > l && (L = l - $),
   e.isHorizontal()
    ? ((S.style.transform = `translate3d(${$}px, 0, 0)`),
      (S.style.width = `${L}px`))
    : ((S.style.transform = `translate3d(0px, ${$}px, 0)`),
      (S.style.height = `${L}px`)),
   k.hide &&
    (clearTimeout(c),
    (C.style.opacity = 1),
 ...
 function v() {
  ...
  (b.style.width = ''),
   (b.style.height = ''),
   (l = e.isHorizontal() ? S.offsetWidth : S.offsetHeight),
   (d =
    e.size /
    (e.virtualSize +
   ...
   e.isHorizontal() ? (b.style.width = `${o}px`) : (b.style.height = `${o}px`),
   d >= 1 ? (S.style.display = 'none') : (S.style.display = ''),
   e.params.scrollbar.hide && (S.style.opacity = 0),
   e.params.watchOverflow &&
    e.enabled &&
    x.el.classList[e.isLocked ? 'add' : 'remove'](e.params.scrollbar.lockClass);
 }
 function g(x) {
 ...
 function u(x) {
  ...
  (k =
   (g(x) -
    ve(C)[e.isHorizontal() ? 'left' : 'top'] -
    (p !== null ? p : o / 2)) /
   (l - o)),
   (k = Math.max(Math.min(k, 1), 0)),
   S && (k = 1 - k);
  const D = e.minTranslate() + (e.maxTranslate() - e.minTranslate()) * k;
  e.updateProgress(D),
   e.setTranslate(D),
   e.updateActiveIndex(),
   e.updateSlidesClasses();
 ...
 function I(x) {
  const { scrollbar: b, params: S } = e,
   C = b.el;
  if (!C) return;
  const k = C,
   D = S.passiveListeners ? { passive: !1, capture: !1 } : !1,
   L = S.passiveListeners ? { passive: !0, capture: !1 } : !1;
  if (!k) return;
  const $ = x === 'on' ? 'addEventListener' : 'removeEventListener';
  k[$]('pointerdown', y, D), r[$]('pointermove', w, D), r[$]('pointerup', E, L);
 }
 function P() {
 ...
 function M() {
  ...
  let C;
  if (
   (typeof S.el == 'string' && e.isElement && (C = e.el.querySelector(S.el)),
   !C && typeof S.el == 'string')
  ) {
   if (((C = r.querySelectorAll(S.el)), !C.length)) return;
  } else C || (C = S.el);
  e.params.uniqueNavElements &&
   typeof S.el == 'string' &&
   C.length > 1 &&
   b.querySelectorAll(S.el).length === 1 &&
   (C = b.querySelector(S.el)),
   C.length > 0 && (C = C[0]),
   C.classList.add(e.isHorizontal() ? S.horizontalClass : S.verticalClass);
  let k;
  C &&
   ((k = C.querySelector(Z(e.params.scrollbar.dragClass))),
   k || ((k = q('div', e.params.scrollbar.dragClass)), C.append(k))),
   Object.assign(x, { el: C, dragEl: k }),
   S.draggable && P(),
   C &&
 ...
 function A() {
  ...
  b &&
   b.classList.remove(
    ...Q(e.isHorizontal() ? x.horizontalClass : x.verticalClass),
   ),
   O();
 }
 i('changeDirection', () => {
  ...
  (b = B(b)),
   b.forEach((S) => {
    S.classList.remove(x.horizontalClass, x.verticalClass),
     S.classList.add(e.isHorizontal() ? x.horizontalClass : x.verticalClass);
   });
 }),
  i('init', () => {
   e.params.scrollbar.enabled === !1 ? z() : (M(), v(), m());
  }),
...
function vs(s) {
 ...
 const a =
  ...
  r = (n, p) => {
   ...
   if (
    ...
    m.indexOf('%') >= 0
     ? (m = `${parseInt(m, 10) * p * l}%`)
     : (m = `${m * p * l}px`),
    h.indexOf('%') >= 0 ? (h = `${parseInt(h, 10) * p}%`) : (h = `${h * p}px`),
    typeof g < 'u' && g !== null)
   ) {
    const w = g - (g - 1) * (1 - Math.abs(p));
    n.style.opacity = w;
   }
   let y = `translate3d(${m}, ${h}, 0px)`;
   if (typeof v < 'u' && v !== null) {
    const w = v - (v - 1) * (1 - Math.abs(p));
    y += ` scale(${w})`;
   }
  ...
  f = () => {
   const { el: n, slides: p, progress: o, snapGrid: l, isElement: d } = e,
    m = Y(n, a);
   e.isElement && m.push(...Y(e.hostEl, a)),
    m.forEach((h) => {
     r(h, o);
    }),
    p.forEach((h, v) => {
     ...
     e.params.slidesPerGroup > 1 &&
      ...
      h.querySelectorAll(`${a}, [data-swiper-parallax-rotate]`).forEach((u) => {
       r(u, g);
      });
    });
  },
  c = function (n) {
   ...
   const { el: p, hostEl: o } = e,
    l = [...p.querySelectorAll(a)];
...
function ws(s) {
 ...
 function v() {
  ...
  const D = o[0].pageX,
   ...
   $ = o[1].pageX,
   G = o[1].pageY;
  return Math.sqrt(($ - D) ** 2 + (G - L) ** 2);
 }
 function g() {
  if (o.length < 2) return { x: null, y: null };
  const D = l.imageEl.getBoundingClientRect();
  return [
   (o[0].pageX + (o[1].pageX - o[0].pageX) / 2 - D.x - r.scrollX) / f,
   (o[0].pageY + (o[1].pageY - o[0].pageY) / 2 - D.y - r.scrollY) / f,
  ];
 }
 function u() {
 ...
 function y(D) {
  ...
  return !!(
   D.target.matches(L) ||
   e.slides.filter(($) => $.contains(D.target)).length > 0
  );
 }
 function w(D) {
  ...
  return !!(
   D.target.matches(L) ||
   [...e.hostEl.querySelectorAll(L)].filter(($) => $.contains(D.target))
    .length > 0
  );
 }
 function E(D) {
  if ((D.pointerType === 'mouse' && o.splice(0, o.length), !y(D))) return;
  const L = e.params.zoom;
  if (((n = !1), (p = !1), o.push(D), !(o.length < 2))) {
   if (((n = !0), (l.scaleStart = v()), !l.slideEl)) {
    (l.slideEl = D.target.closest(`.${e.params.slideClass}, swiper-slide`)),
     l.slideEl || (l.slideEl = e.slides[e.activeIndex]);
    let $ = l.slideEl.querySelector(`.${L.containerClass}`);
    if (
     ($ &&
      ($ = $.querySelectorAll(
     ...
     (l.imageEl = $),
     $
      ? (l.imageWrapEl = te(l.imageEl, `.${L.containerClass}`)[0])
    ...
    l.maxRatio = l.imageWrapEl.getAttribute('data-swiper-zoom') || L.maxRatio;
   }
   if (l.imageEl) {
    const [$, G] = g();
    (l.originX = $),
 ...
 function I(D) {
  ...
  const L = e.params.zoom,
   $ = e.zoom,
   G = o.findIndex((N) => N.pointerId === D.pointerId);
  G >= 0 && (o[G] = D),
   !(o.length < 2) &&
    ((p = !0),
    (l.scaleMove = v()),
    l.imageEl &&
     (($.scale = (l.scaleMove / l.scaleStart) * f),
     $.scale > l.maxRatio &&
      ($.scale = l.maxRatio - 1 + ($.scale - l.maxRatio + 1) ** 0.5),
     $.scale < L.minRatio &&
      ($.scale = L.minRatio + 1 - (L.minRatio - $.scale + 1) ** 0.5),
     (l.imageEl.style.transform = `translate3d(0,0,0) scale(${$.scale})`)));
 }
 function P(D) {
  if (!y(D) || (D.pointerType === 'mouse' && D.type === 'pointerout')) return;
  const L = e.params.zoom,
   $ = e.zoom,
   G = o.findIndex((N) => N.pointerId === D.pointerId);
  G >= 0 && o.splice(G, 1),
   !(!n || !p) &&
    ...
    (p = !1),
    l.imageEl &&
     (($.scale = Math.max(Math.min($.scale, l.maxRatio), L.minRatio)),
     (l.imageEl.style.transitionDuration = `${e.params.speed}ms`),
     (l.imageEl.style.transform = `translate3d(0,0,0) scale(${$.scale})`),
     (f = $.scale),
     (c = !1),
     $.scale > 1 && l.slideEl
      ? l.slideEl.classList.add(`${L.zoomedSlideClass}`)
      : $.scale <= 1 &&
        l.slideEl &&
        l.slideEl.classList.remove(`${L.zoomedSlideClass}`),
     $.scale === 1 &&
 ...
 function O(D) {
  ...
  L.android && D.cancelable && D.preventDefault(), (d.isTouched = !0);
  const $ = o.length > 0 ? o[0] : D;
 ...
 function M(D) {
  ...
  d.isMoved ||
   ...
   (d.startX = Ie(l.imageWrapEl, 'x') || 0),
   (d.startY = Ie(l.imageWrapEl, 'y') || 0),
   (l.slideWidth = l.slideEl.offsetWidth),
   (l.slideHeight = l.slideEl.offsetHeight),
   (l.imageWrapEl.style.transitionDuration = '0ms'));
  const $ = d.width * L.scale,
   G = d.height * L.scale;
  if ($ < l.slideWidth && G < l.slideHeight) return;
  if (
   ((d.minX = Math.min(l.slideWidth / 2 - $ / 2, 0)),
   (d.maxX = -d.minX),
   (d.minY = Math.min(l.slideHeight / 2 - G / 2, 0)),
   (d.maxY = -d.minY),
   (d.touchesCurrent.x = o.length > 0 ? o[0].pageX : D.pageX),
   (d.touchesCurrent.y = o.length > 0 ? o[0].pageY : D.pageY),
   Math.max(
  ...
  D.cancelable && D.preventDefault(), D.stopPropagation(), (d.isMoved = !0);
  const F = (L.scale - f) / (l.maxRatio - e.params.zoom.minRatio),
   { originX: H, originY: X } = l;
  (d.currentX =
   d.touchesCurrent.x - d.touchesStart.x + d.startX + F * (d.width - H * 2)),
   (d.currentY =
    d.touchesCurrent.y - d.touchesStart.y + d.startY + F * (d.height - X * 2)),
   d.currentX < d.minX &&
 ...
 function A() {
  ...
  const G = m.x * L,
   ...
   F = m.y * $,
   H = d.currentY + F;
  m.x !== 0 && (L = Math.abs((N - d.currentX) / m.x)),
   m.y !== 0 && ($ = Math.abs((H - d.currentY) / m.y));
  const X = Math.max(L, $);
  (d.currentX = N), (d.currentY = H);
  const j = d.width * D.scale,
   K = d.height * D.scale;
 ...
 function z(D) {
   ...
   D &&
    ...
    l.slideEl ||
     (e.params.virtual && e.params.virtual.enabled && e.virtual
      ? (l.slideEl = Y(e.slidesEl, `.${e.params.slideActiveClass}`)[0])
      : (l.slideEl = e.slides[e.activeIndex]));
   let le = l.slideEl.querySelector(`.${$.containerClass}`);
   le &&
    ...
    (l.imageEl = le),
    le
     ? (l.imageWrapEl = te(l.imageEl, `.${$.containerClass}`)[0])
     : (l.imageWrapEl = void 0);
  }
  ...
  let G, N, F, H, X, j, K, ee, Oe, ke, Ge, He, pe, me, ye, be, Se, Ee;
  typeof d.touchesStart.x > 'u' && D
   ? ((G = D.pageX), (N = D.pageY))
   : ((G = d.touchesStart.x), (N = d.touchesStart.y));
  const ne = typeof D == 'number' ? D : null;
  f === 1 && ne && ((G = void 0), (N = void 0)),
   (L.scale =
    ne || l.imageWrapEl.getAttribute('data-swiper-zoom') || $.maxRatio),
   (f = ne || l.imageWrapEl.getAttribute('data-swiper-zoom') || $.maxRatio),
   D && !(f === 1 && ne)
    ? ((Se = l.slideEl.offsetWidth),
      (Ee = l.slideEl.offsetHeight),
      (F = ve(l.slideEl).left + r.scrollX),
      (H = ve(l.slideEl).top + r.scrollY),
      (X = F + Se / 2 - G),
      (j = H + Ee / 2 - N),
      (Oe = l.imageEl.offsetWidth),
      (ke = l.imageEl.offsetHeight),
      (Ge = Oe * L.scale),
      (He = ke * L.scale),
      (pe = Math.min(Se / 2 - Ge / 2, 0)),
      (me = Math.min(Ee / 2 - He / 2, 0)),
      (ye = -pe),
      (be = -me),
      (K = X * L.scale),
      (ee = j * L.scale),
      ...
      ee > be && (ee = be))
    : ((K = 0), (ee = 0)),
   ne && L.scale === 1 && ((l.originX = 0), (l.originY = 0)),
   ...
   (l.imageEl.style.transform = `translate3d(0,0,0) scale(${L.scale})`);
 }
 function x() {
  const D = e.zoom,
   L = e.params.zoom;
 ...
 function b(D) {
  const L = e.zoom;
  L.scale && L.scale !== 1 ? x() : z(D);
 }
 function S() {
  const D = e.params.passiveListeners ? { passive: !0, capture: !1 } : !1,
   L = e.params.passiveListeners ? { passive: !1, capture: !0 } : !0;
  return { passiveListener: D, activeListenerWithCapture: L };
 }
 function C() {
  const D = e.zoom;
  if (D.enabled) return;
  D.enabled = !0;
  const { passiveListener: L, activeListenerWithCapture: $ } = S();
  e.wrapperEl.addEventListener('pointerdown', E, L),
   e.wrapperEl.addEventListener('pointermove', I, $),
   ['pointerup', 'pointercancel', 'pointerout'].forEach((G) => {
    e.wrapperEl.addEventListener(G, P, L);
   }),
 ...
 function k() {
  ...
  D.enabled = !1;
  const { passiveListener: L, activeListenerWithCapture: $ } = S();
  e.wrapperEl.removeEventListener('pointerdown', E, L),
   e.wrapperEl.removeEventListener('pointermove', I, $),
   ['pointerup', 'pointercancel', 'pointerout'].forEach((G) => {
    e.wrapperEl.removeEventListener(G, P, L);
   }),
   e.wrapperEl.removeEventListener('pointermove', M, $);
 }
 i('init', () => {
  e.params.zoom.enabled && C();
 }),
  i('destroy', () => {
   k();
  }),
  ...
  Object.assign(e.zoom, { enable: C, disable: k, in: z, out: x, toggle: b });
}
function ys(s) {
 let { swiper: e, extendParams: t, on: i } = s;
 ...
 function a(p, o) {
  const l = (function () {
   let v, g, u;
   return (y, w) => {
    for (g = -1, v = y.length; v - g > 1; )
     (u = (v + g) >> 1), y[u] <= w ? (g = u) : (v = u);
    return v;
   };
  })();
  (this.x = p), (this.y = o), (this.lastIndex = p.length - 1);
  let d, m;
  return (
   (this.interpolate = function (v) {
    return v
     ? ((m = l(this.x, v)),
       (d = m - 1),
       ((v - this.x[d]) * (this.y[m] - this.y[d])) / (this.x[m] - this.x[d]) +
        this.y[d])
     : 0;
   }),
   this
 ...
 function c(p, o) {
  ...
  if (Array.isArray(d))
   for (m = 0; m < d.length; m += 1) d[m] !== o && d[m] instanceof l && h(d[m]);
   ...
   if (p && p.swiper) e.controller.control = p.swiper;
    ...
    const o = (l) => {
     (e.controller.control = l.detail[0]),
      e.update(),
      p.removeEventListener('init', o);
    };
    p.addEventListener('init', o);
   }
   return;
  }
...
function bs(s) {
 ...
 function r(b) {
  const S = a;
  S.length !== 0 && ((S.innerHTML = ''), (S.innerHTML = b));
 }
 function f(b) {
  b === void 0 && (b = 16);
 ...
 function p(b, S) {
  (b = B(b)),
   b.forEach((C) => {
    C.setAttribute('role', S);
   });
 }
 function o(b, S) {
  (b = B(b)),
   b.forEach((C) => {
    C.setAttribute('aria-roledescription', S);
   });
 }
 function l(b, S) {
  (b = B(b)),
   b.forEach((C) => {
    C.setAttribute('aria-controls', S);
   });
 }
 function d(b, S) {
  (b = B(b)),
 ...
 function u(b) {
  ...
  (e.pagination &&
   e.pagination.el &&
   (C === e.pagination.el || e.pagination.el.contains(b.target)) &&
   !b.target.matches(Z(e.params.pagination.bulletClass))) ||
   (e.navigation &&
    e.navigation.nextEl &&
    C === e.navigation.nextEl &&
    ((e.isEnd && !e.params.loop) || e.slideNext(),
    e.isEnd ? r(S.lastSlideMessage) : r(S.nextSlideMessage)),
   e.navigation &&
    e.navigation.prevEl &&
    C === e.navigation.prevEl &&
    ((e.isBeginning && !e.params.loop) || e.slidePrev(),
    e.isBeginning ? r(S.firstSlideMessage) : r(S.prevSlideMessage)),
   e.pagination && C.matches(Z(e.params.pagination.bulletClass)) && C.click());
 }
 function y() {
  if (e.params.loop || e.params.rewind || !e.navigation) return;
  const { nextEl: b, prevEl: S } = e.navigation;
  S && (e.isBeginning ? (v(S), n(S)) : (g(S), c(S))),
   b && (e.isEnd ? (v(b), n(b)) : (g(b), c(b)));
 }
 function w() {
   ...
   e.pagination.bullets.forEach((S) => {
    e.params.pagination.clickable &&
     ...
     e.params.pagination.renderBullet ||
      (p(S, 'button'),
      d(S, b.paginationBulletMessage.replace(/\{\{index\}\}/, fe(S) + 1)))),
     S.matches(Z(e.params.pagination.bulletActiveClass))
 ...
 const P = (b, S, C) => {
   c(b),
    b.tagName !== 'BUTTON' &&
     (p(b, 'button'), b.addEventListener('keydown', u)),
    d(b, C),
    l(b, S);
  },
  O = () => {
  ...
  A = (b) => {
   ...
   const S = b.target.closest(`.${e.params.slideClass}, swiper-slide`);
   if (!S || !e.slides.includes(S)) return;
   const C = e.slides.indexOf(S) === e.activeIndex,
    k =
  ...
  T = () => {
   const b = e.params.a11y;
   b.itemRoleDescriptionMessage && o(e.slides, b.itemRoleDescriptionMessage),
    b.slideRole && p(e.slides, b.slideRole);
   const S = e.slides.length;
   b.slideLabelMessage &&
    e.slides.forEach((C, k) => {
     const D = e.params.loop
       ? parseInt(C.getAttribute('data-swiper-slide-index'), 10)
       : k,
      L = b.slideLabelMessage
       .replace(/\{\{index\}\}/, D + 1)
  ...
  z = () => {
   ...
   e.el.append(a);
   const S = e.el;
   b.containerRoleDescriptionMessage && o(S, b.containerRoleDescriptionMessage),
    b.containerMessage && d(S, b.containerMessage);
   const C = e.wrapperEl,
    k = b.id || C.getAttribute('id') || `swiper-wrapper-${f(16)}`,
    D = e.params.autoplay && e.params.autoplay.enabled ? 'off' : 'polite';
   m(C, k), h(C, D), T();
   let { nextEl: L, prevEl: $ } = e.navigation ? e.navigation : {};
   (L = B(L)),
    ($ = B($)),
    L && L.forEach((G) => P(G, k, b.nextSlideMessage)),
    $ && $.forEach((G) => P(G, k, b.prevSlideMessage)),
    E() &&
     B(e.pagination.el).forEach((N) => {
 ...
 function x() {
  a && a.remove();
  let { nextEl: b, prevEl: S } = e.navigation ? e.navigation : {};
  (b = B(b)),
   (S = B(S)),
   b && b.forEach((C) => C.removeEventListener('keydown', u)),
   S && S.forEach((C) => C.removeEventListener('keydown', u)),
   E() &&
    B(e.pagination.el).forEach((k) => {
...
function Ss(s) {
 ...
 const f = (m) =>
  ...
  n = (m, h) => {
   ...
   const u = e.slides[h];
   let y = f(u.getAttribute('data-history'));
   if (e.params.history.root.length > 0) {
    let E = e.params.history.root;
    E[E.length - 1] === '/' && (E = E.slice(0, E.length - 1)),
     (y = `${E}/${m ? `${m}/` : ''}${y}`);
   } else g.pathname.includes(m) || (y = `${m ? `${m}/` : ''}${y}`);
   e.params.history.keepQuery && (y += g.search);
   const w = v.history.state;
   (w && w.value === y) ||
    (e.params.history.replaceState
     ? v.history.replaceState({ value: y }, null, y)
     : v.history.pushState({ value: y }, null, y));
  },
  p = (m, h, v) => {
   if (h)
    for (let g = 0, u = e.slides.length; g < u; g += 1) {
     const y = e.slides[g];
     if (f(y.getAttribute('data-history')) === h) {
      const E = e.getSlideIndex(y);
      e.slideTo(E, m, v);
     }
    }
  ...
  o = () => {
   (r = c(e.params.url)), p(e.params.speed, r.value, !1);
  },
  l = () => {
   const m = R();
   if (e.params.history) {
...
function Es(s) {
 ...
 const n = () => {
  ...
  l = () => {
   e.params.hashNavigation.watchState && c.removeEventListener('hashchange', n);
  };
 a('init', () => {
...
function xs(s) {
 ...
 function w(L) {
  !e ||
   ...
   (L.target === e.wrapperEl &&
    (e.wrapperEl.removeEventListener('transitionend', w), !y && T()));
 }
 const E = () => {
   if (e.destroyed || !e.autoplay.running) return;
   e.autoplay.paused ? (d = !0) : d && ((p = o), (d = !1));
   const L = e.autoplay.paused ? o : l + p - new Date().getTime();
  ...
  P = (L) => {
   ...
   let $ = typeof L > 'u' ? e.params.autoplay.delay : L;
   (n = e.params.autoplay.delay), (p = e.params.autoplay.delay);
   const G = I();
   !Number.isNaN(G) && G > 0 && typeof L > 'u' && (($ = G), (n = G), (p = G)),
    (o = $);
   const N = e.params.speed,
    F = () => {
     !e ||
      ...
      (e.params.autoplay.reverseDirection
       ? !e.isBeginning || e.params.loop || e.params.rewind
         ...
         : e.params.autoplay.stopOnLastSlide ||
           (e.slideTo(e.slides.length - 1, N, !0, !0), a('autoplay'))
       : !e.isEnd || e.params.loop || e.params.rewind
  ...
  z = () => {
   if (e.destroyed || !e.autoplay.running) return;
   const L = V();
   L.visibilityState === 'hidden' && ((u = !0), A(!0)),
    L.visibilityState === 'visible' && T();
  },
  x = (L) => {
   L.pointerType === 'mouse' &&
    ((u = !0), (y = !0), !(e.animating || e.autoplay.paused) && A(!0));
  },
  b = (L) => {
   L.pointerType === 'mouse' && ((y = !1), e.autoplay.paused && T());
  },
  S = () => {
 ...
 i('init', () => {
  e.params.autoplay.enabled && (S(), k(), O());
 }),
  i('destroy', () => {
   C(), D(), e.autoplay.running && M();
  }),
  i('_freeModeStaticRelease', () => {
   (h || u) && T();
  }),
  i('_freeModeNoMomentumRelease', () => {
   e.params.autoplay.disableOnInteraction ? M() : A(!0, !0);
  }),
  i('beforeTransitionStart', (L, $, G) => {
   e.destroyed ||
    !e.autoplay.running ||
    (G || !e.params.autoplay.disableOnInteraction ? A(!0, !0) : M());
  }),
  i('sliderFirstMove', () => {
  ...
  i('touchEnd', () => {
   if (!(e.destroyed || !e.autoplay.running || !m)) {
    if (
     (clearTimeout(v), clearTimeout(f), e.params.autoplay.disableOnInteraction)
    ) {
     (h = !1), (m = !1);
     return;
    }
  ...
  Object.assign(e.autoplay, { start: O, stop: M, pause: A, resume: T });
}
function Ts(s) {
 let { swiper: e, extendParams: t, on: i } = s;
 t({
 ...
 function f() {
  ...
  let d;
  p.params.loop
   ? (d = parseInt(p.clickedSlide.getAttribute('data-swiper-slide-index'), 10))
   : (d = o),
 ...
 function n(p) {
  ...
  if (
   ...
   for (let g = 0; g < d; g += 1)
    Y(o.slidesEl, `[data-swiper-slide-index="${e.realIndex + g}"]`).forEach(
     (u) => {
      u.classList.add(m);
     },
    );
    ...
    o.visibleSlidesIndexes &&
     ...
     (o.params.centeredSlides
      ? u > g
        ...
        : (u = u + Math.floor(l / 2) - 1)
      : u > g && o.params.slidesPerGroup,
     o.slideTo(u, p ? 0 : void 0));
  }
 }
 i('beforeInit', () => {
  const { thumbs: p } = e.params;
  if (!(!p || !p.swiper))
   if (typeof p.swiper == 'string' || p.swiper instanceof HTMLElement) {
    const o = V(),
     l = () => {
      const m =
       typeof p.swiper == 'string' ? o.querySelector(p.swiper) : p.swiper;
      if (m && m.swiper) (p.swiper = m.swiper), c(), n(!0);
      else if (m) {
       const h = (v) => {
        (p.swiper = v.detail[0]),
  ...
  if (o.freeMode.momentum) {
   if (h.velocities.length > 1) {
    const M = h.velocities.pop(),
     A = h.velocities.pop(),
     T = M.position - A.position,
     z = M.time - A.time;
    (e.velocity = T / z),
     (e.velocity /= 2),
     Math.abs(e.velocity) < o.freeMode.minimumVelocity && (e.velocity = 0),
     (z > 150 || U() - M.time > 300) && (e.velocity = 0);
   } else e.velocity = 0;
   (e.velocity *= o.freeMode.momentumVelocityRatio), (h.velocities.length = 0);
   let u = 1e3 * o.freeMode.momentumRatio;
   const y = e.velocity * u;
   let w = e.translate + y;
   d && (w = -w);
   let E = !1,
    I;
   const P = Math.abs(e.velocity) * 20 * o.freeMode.momentumBounceRatio;
   let O;
   if (w < e.maxTranslate())
    o.freeMode.momentumBounce
     ? (w + e.maxTranslate() < -P && (w = e.maxTranslate() - P),
       (I = e.maxTranslate()),
       (E = !0),
    ...
    o.freeMode.momentumBounce
     ? (w - e.minTranslate() > P && (w = e.minTranslate() + P),
       (I = e.minTranslate()),
       (E = !0),
       (h.allowMomentumBounce = !0))
     : (w = e.minTranslate()),
    ...
    Math.abs(m[M] - w) < Math.abs(m[M - 1] - w) || e.swipeDirection === 'next'
     ? (w = m[M])
     : (w = m[M - 1]),
     (w = -w);
   }
   if (
    ...
    if (
     (d
      ? (u = Math.abs((-w - e.translate) / e.velocity))
      : (u = Math.abs((w - e.translate) / e.velocity)),
     o.freeMode.sticky)
    ) {
     const M = Math.abs((d ? -w : w) - e.translate),
 ...
 const n = () => {
  ...
  p = (v) => {
   const { slidesPerView: g } = e.params,
    { rows: u, fill: y } = e.params.grid,
    w =
     e.virtual && e.params.virtual.enabled ? e.virtual.slides.length : v.length;
   (f = Math.floor(w / u)),
    Math.floor(w / u) === w / u ? (a = w) : (a = Math.ceil(w / u) * u),
    g !== 'auto' && y === 'row' && (a = Math.max(a, g * u)),
    (r = a / u);
  },
  o = () => {
   e.slides &&
  ...
  l = (v, g, u) => {
   const { slidesPerGroup: y } = e.params,
    w = n(),
    { rows: E, fill: I } = e.params.grid,
    P =
     e.virtual && e.params.virtual.enabled ? e.virtual.slides.length : u.length;
   let O, M, A;
   if (I === 'row' && y > 1) {
    const T = Math.floor(v / (y * E)),
     z = v - E * y * T,
     x = T === 0 ? y : Math.min(Math.ceil((P - T * E * y) / E), y);
    (A = Math.floor(z / x)),
     (M = z - A * x + T * y),
     (O = M + (A * a) / E),
     (g.style.order = O);
    ...
    I === 'column'
     ? ((M = Math.floor(v / E)),
       ...
       (M > f || (M === f && A === E - 1)) &&
        ((A += 1), A >= E && ((A = 0), (M += 1))))
     : ((A = Math.floor(v / r)), (M = v - A * r));
   (g.row = A),
    (g.column = M),
    (g.style.height = `calc((100% - ${(E - 1) * w}px) / ${E})`),
    (g.style[e.getDirectionLabel('margin-top')] = A !== 0 ? w && `${w}px` : ''),
  ...
  d = (v, g) => {
   ...
   if (
    ((e.virtualSize = (v + w) * a),
    (e.virtualSize = Math.ceil(e.virtualSize / E) - w),
    e.params.cssMode ||
    ...
    for (let P = 0; P < g.length; P += 1) {
     let O = g[P];
     y && (O = Math.floor(O)), g[P] < e.virtualSize + g[0] && I.push(O);
    }
...
function Is(s, e) {
 ...
 for (let o = 0; o < p.length; o += 1) r.append(p[o]);
 t.recalcSlides(),
  i.loop && t.loopCreate(),
...
function $s(s) {
 let { swiper: e } = s;
 Object.assign(e, {
  appendSlide: Ps.bind(e),
  prependSlide: Ls.bind(e),
  addSlide: Is.bind(e),
  removeSlide: zs.bind(e),
...
function we(s) {
 ...
 const { activeIndex: r } = e,
  f = (c) =>
   c.parentElement
    ? c.parentElement
    : e.slides.filter((p) => p.shadowRoot && p.shadowRoot === c.parentNode)[0];
 if (e.params.virtualTranslate && t !== 0) {
  ...
  a
   ? (n = i)
   : (n = i.filter((p) => {
      const o = p.classList.contains('swiper-slide-transform') ? f(p) : p;
...
function Ds(s) {
 ...
 t({ fadeEffect: { crossFade: !1 } }),
  ae({
   ...
   setTransition: (f) => {
    const c = e.slides.map((n) => ie(n));
    c.forEach((n) => {
     n.style.transitionDuration = `${f}ms`;
    }),
     we({ swiper: e, duration: f, transformElements: c, allSlides: !0 });
   },
   overwriteParams: () => ({
    slidesPerView: 1,
    slidesPerGroup: 1,
...
function Os(s) {
 ...
 const a = (n, p, o) => {
  ...
  l ||
   ((l = q(
    'div',
    `swiper-slide-shadow-cube swiper-slide-shadow-${o ? 'left' : 'top'}`.split(
     ' ',
    ),
   )),
   n.append(l)),
   d ||
    ((d = q(
     'div',
     `swiper-slide-shadow-cube swiper-slide-shadow-${o ? 'right' : 'bottom'}`.split(
      ' ',
     ),
    )),
    n.append(d)),
   ...
   g.shadow &&
    (u
     ? ((E = e.wrapperEl.querySelector('.swiper-cube-shadow')),
       E || ((E = q('div', 'swiper-cube-shadow')), e.wrapperEl.append(E)),
       (E.style.height = `${l}px`))
     : ((E = n.querySelector('.swiper-cube-shadow')),
       E || ((E = q('div', 'swiper-cube-shadow')), n.append(E))));
   for (let P = 0; P < o.length; P += 1) {
    const O = o[P];
    let M = P;
    y && (M = parseInt(O.getAttribute('data-swiper-slide-index'), 10));
    let A = M * 90,
     T = Math.floor(A / 360);
    m && ((A = -A), (T = Math.floor(-A / 360)));
    const z = Math.max(Math.min(O.progress, 1), -1);
    let x = 0,
     b = 0,
     S = 0;
    M % 4 === 0
     ...
     : (M - 1) % 4 === 0
       ...
       : (M - 2) % 4 === 0
         ? ((x = h + T * 4 * h), (S = h))
         : (M - 3) % 4 === 0 && ((x = -h), (S = 3 * h + h * 4 * T)),
     m && (x = -x),
     u || ((b = x), (x = 0));
    const C = `rotateX(${u ? 0 : -A}deg) rotateY(${u ? A : 0}deg) translate3d(${x}px, ${b}px, ${S}px)`;
    z <= 1 &&
     z > -1 &&
     ((w = M * 90 + z * 90),
     m && (w = -M * 90 - z * 90),
     e.browser &&
      ...
      (w += 0.001)),
     (O.style.transform = C),
     g.slideShadows && a(O, z, u);
   }
   if (
    ((p.style.transformOrigin = `50% 50% -${h / 2}px`),
    (p.style['-webkit-transform-origin'] = `50% 50% -${h / 2}px`),
    g.shadow)
   )
    if (u)
     E.style.transform = `translate3d(0px, ${l / 2 + g.shadowOffset}px, ${-l / 2}px) rotateX(89.99deg) rotateZ(0deg) scale(${g.shadowScale})`;
    else {
     const P = Math.abs(w) - Math.floor(Math.abs(w) / 90) * 90,
      O =
       1.5 -
       (Math.sin((P * 2 * Math.PI) / 360) / 2 +
        Math.cos((P * 2 * Math.PI) / 360) / 2),
      M = g.shadowScale,
      A = g.shadowScale / O,
      T = g.shadowOffset;
     E.style.transform = `scale3d(${M}, 1, ${A}) translate3d(0px, ${d / 2 + T}px, ${-d / 2 / A}px) rotateX(-89.99deg)`;
    }
   const I = (v.isSafari || v.isWebView) && v.needPerspectiveFix ? -h / 2 : 0;
   (p.style.transform = `translate3d(0px,0,${I}px) rotateX(${e.isHorizontal() ? 0 : w}deg) rotateY(${e.isHorizontal() ? -w : 0}deg)`),
    p.style.setProperty('--swiper-cube-translate-z', `${I}px`);
  },
  setTransition: (n) => {
 ...
 const a = (n, p) => {
  let o = e.isHorizontal()
   ...
   l = e.isHorizontal()
    ? n.querySelector('.swiper-slide-shadow-right')
    : n.querySelector('.swiper-slide-shadow-bottom');
  o || (o = re('flip', n, e.isHorizontal() ? 'left' : 'top')),
   l || (l = re('flip', n, e.isHorizontal() ? 'right' : 'bottom')),
 ...
 ae({
  ...
  setTranslate: () => {
   ...
   for (let l = 0; l < n.length; l += 1) {
    ...
    const h = d.swiperSlideOffset;
    let g = -180 * m,
     u = 0,
     y = e.params.cssMode ? -h - e.translate : -h,
     w = 0;
    e.isHorizontal() ? p && (g = -g) : ((w = y), (y = 0), (u = -g), (g = 0)),
     e.browser &&
      e.browser.need3dFix &&
    ...
    const E = `translate3d(${y}px, ${w}px, 0px) rotateX(${u}deg) rotateY(${g}deg)`,
     I = ue(o, d);
    I.style.transform = E;
   }
  },
  setTransition: (n) => {
   const p = e.slides.map((o) => ie(o));
   p.forEach((o) => {
    ...
    we({ swiper: e, duration: n, transformElements: p });
  },
  recreateShadows: () => {
   e.params.flipEffect,
    e.slides.forEach((n) => {
...
function Gs(s) {
 ...
 t({
  ...
  ae({
   ...
   on: i,
   setTranslate: () => {
    const { width: f, height: c, slides: n, slidesSizesGrid: p } = e,
     ...
     v = o.depth;
    for (let g = 0, u = n.length; g < u; g += 1) {
     const y = n[g],
      w = p[g],
      E = y.swiperSlideOffset,
      I = (m - E - w / 2) / w,
      P = typeof o.modifier == 'function' ? o.modifier(I) : I * o.modifier;
     let O = l ? h * P : 0,
      M = l ? 0 : h * P,
      A = -v * Math.abs(P),
      T = o.stretch;
     typeof T == 'string' &&
      T.indexOf('%') !== -1 &&
      (T = (parseFloat(o.stretch) / 100) * w);
     let z = l ? 0 : T * P,
      x = l ? T * P : 0,
      b = 1 - (1 - o.scale) * Math.abs(P);
     Math.abs(x) < 0.001 && (x = 0),
      ...
      e.browser &&
       e.browser.need3dFix &&
       ((Math.abs(O) / 90) % 2 === 1 && (O += 0.001),
       (Math.abs(M) / 90) % 2 === 1 && (M += 0.001));
     const S = `translate3d(${x}px,${z}px,${A}px)  rotateX(${M}deg) rotateY(${O}deg) scale(${b})`,
      C = ue(o, y);
     if (
      ((C.style.transform = S),
      (y.style.zIndex = -Math.abs(Math.round(P)) + 1),
      o.slideShadows)
     ) {
      let k = l
      ...
      k || (k = re('coverflow', y, l ? 'left' : 'top')),
       D || (D = re('coverflow', y, l ? 'right' : 'bottom')),
       k && (k.style.opacity = P > 0 ? P : 0),
       D && (D.style.opacity = -P > 0 ? -P : 0);
     }
    }
   },
   setTransition: (f) => {
...
function Hs(s) {
 ...
 ae({
  ...
  on: i,
  setTranslate: () => {
   const { slides: c, wrapperEl: n, slidesSizesGrid: p } = e,
   ...
   for (let m = 0; m < c.length; m += 1) {
    const h = c[m],
     v = h.progress,
     g = Math.min(Math.max(h.progress, -o.limitProgress), o.limitProgress);
    let u = g;
    d ||
     (u = Math.min(
    ...
    const y = h.swiperSlideOffset,
     w = [e.params.cssMode ? -y - e.translate : -y, 0, 0],
     E = [0, 0, 0];
    let I = !1;
    e.isHorizontal() || ((w[1] = w[0]), (w[0] = 0));
    let P = { translate: [0, 0, 0], rotate: [0, 0, 0], scale: 1, opacity: 1 };
    g < 0 ? ((P = o.next), (I = !0)) : g > 0 && ((P = o.prev), (I = !0)),
     w.forEach((b, S) => {
      w[S] = `calc(${b}px + (${a(P.translate[S])} * ${Math.abs(g * l)}))`;
     }),
     E.forEach((b, S) => {
      let C = P.rotate[S] * Math.abs(g * l);
      e.browser &&
       e.browser.need3dFix &&
       (Math.abs(C) / 90) % 2 === 1 &&
       (C += 0.001),
    ...
    const O = w.join(', '),
     ...
     A =
      u < 0
       ? `scale(${1 + (1 - P.scale) * u * l})`
       : `scale(${1 - (1 - P.scale) * u * l})`,
     T = u < 0 ? 1 + (1 - P.opacity) * u * l : 1 - (1 - P.opacity) * u * l,
     z = `translate3d(${O}) ${M} ${A}`;
    if ((I && P.shadow) || !I) {
     let b = h.querySelector('.swiper-slide-shadow');
     if ((!b && P.shadow && (b = re('creative', h)), b)) {
      const S = o.shadowPerProgress ? g * (1 / o.limitProgress) : g;
      b.style.opacity = Math.min(Math.max(Math.abs(S), 0), 1);
     }
    }
    const x = ue(o, h);
    (x.style.transform = z),
     (x.style.opacity = T),
     P.origin && (x.style.transformOrigin = P.origin);
   }
  },
  setTransition: (c) => {
   const n = e.slides.map((p) => ie(p));
   n.forEach((p) => {
    ...
    we({ swiper: e, duration: c, transformElements: n, allSlides: !0 });
  },
  ...
  ae({
   ...
   setTranslate: () => {
    ...
    for (let m = 0; m < f.length; m += 1) {
     ...
     e.params.centeredSlides &&
      ...
      e.params.centeredSlides &&
       e.params.cssMode &&
       (u -= f[0].swiperSlideOffset);
     let y = e.params.cssMode ? -u - e.translate : -u,
      w = 0;
     const E = -100 * Math.abs(g);
     let I = 1,
      P = -p.perSlideRotate * g,
      O = p.perSlideOffset - Math.abs(g) * 0.75;
     const M = e.virtual && e.params.virtual.enabled ? e.virtual.from + m : m,
      A =
       (M === c || M === c - 1) &&
       g > 0 &&
       g < 1 &&
      ...
      T =
       ...
       d > o;
     if (A || T) {
      const S = (1 - Math.abs((Math.abs(g) - 0.5) / 0.5)) ** 0.5;
      (P += -28 * g * S),
       (I += -0.5 * S),
       (O += 96 * S),
       (w = `${-25 * S * Math.abs(g)}%`);
     ...
     const z = g < 0 ? `${1 + (1 - I) * g}` : `${1 - (1 - I) * g}`,
      ...
      `;
     if (p.slideShadows) {
      let S = h.querySelector('.swiper-slide-shadow');
      S || (S = re('cards', h)),
       S &&
        (S.style.opacity = Math.min(Math.max((Math.abs(g) - 0.5) / 0.5, 0), 1));
     }
     h.style.zIndex = -Math.abs(Math.round(v)) + f.length;
     const b = ue(p, h);
     b.style.transform = x;
    }
   },
   setTransition: (f) => {
    ...
    c.forEach((n) => {
     ...
     we({ swiper: e, duration: f, transformElements: c });
   },
   perspective: () => !0,
   overwriteParams: () => ({
    watchSlidesProgress: !0,
