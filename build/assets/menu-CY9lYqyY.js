...
class o {
 constructor(t, n = {}, e = null) {
  if (
   ((this._el = t),
   (this._horizontal = n.orientation === 'horizontal'),
   (this._animate = n.animate !== !1),
   (this._accordion = n.accordion !== !1),
   (this._showDropdownOnHover = !!n.showDropdownOnHover),
   (this._closeChildren = !!n.closeChildren),
   (this._rtl =
    document.documentElement.getAttribute('dir') === 'rtl' ||
    document.body.getAttribute('dir') === 'rtl'),
   (this._onOpen = n.onOpen || (() => {})),
   (this._onOpened = n.onOpened || (() => {})),
   (this._onClose = n.onClose || (() => {})),
   (this._onClosed = n.onClosed || (() => {})),
   (this._psScroll = null),
  ...
  for (let s = 0; s < i.length; s++)
   i[s].startsWith('bg-') && (this._menuBgClass = i[s]);
 ...
 _bindEvents() {
  (this._evntElClick = (t) => {
   ...
   const n = t.target.classList.contains('menu-toggle')
    ? t.target
    : o._findParent(t.target, 'menu-toggle', !1);
   n &&
    (t.preventDefault(),
    n.getAttribute('data-hover') !== 'true' && this.toggle(n));
  }),
   ...
   (this._evntWindowResize = () => {
    this.update(),
     this._lastWidth !== window.innerWidth &&
      ((this._lastWidth = window.innerWidth), this.update());
    const t = document.querySelector("[data-template^='horizontal-menu']");
    !this._horizontal && !t && this.manageScroll();
   }),
   window.addEventListener('resize', this._evntWindowResize),
   this._horizontal &&
    ((this._evntPrevBtnClick = (t) => {
     t.preventDefault(),
      !this._prevBtn.classList.contains('disabled') && this._slide('prev');
    }),
    this._prevBtn.addEventListener('click', this._evntPrevBtnClick),
    (this._evntNextBtnClick = (t) => {
     t.preventDefault(),
      !this._nextBtn.classList.contains('disabled') && this._slide('next');
    }),
    this._nextBtn.addEventListener('click', this._evntNextBtnClick),
    (this._evntBodyClick = (t) => {
     !this._inner.contains(t.target) &&
      this._el.querySelectorAll('.menu-inner > .menu-item.open').length &&
      this.closeAll();
    }),
    document.body.addEventListener('click', this._evntBodyClick),
    this._showDropdownOnHover &&
     ((this._evntElMouseOver = (t) => {
      if (
       t.target !== t.currentTarget &&
       !t.target.parentNode.classList.contains('open')
      ) {
     ...
     (this._evntElMouseOut = (t) => {
      const n = t.currentTarget,
       e = t.target,
       i = t.toElement || t.relatedTarget;
      if (
       (e.closest('ul') &&
        e.closest('ul').classList.contains('menu-inner') &&
        (this._topParent = e),
       e !== n &&
        (e.parentNode.classList.contains('open') ||
         !e.classList.contains('menu-toggle')) &&
        i &&
        i.parentNode &&
        !i.parentNode.classList.contains('menu-link'))
      ) {
       if (this._topParent && !o.childOf(i, this._topParent.parentNode)) {
        const a = this._topParent.classList.contains('menu-toggle')
         ? this._topParent
         : null;
        a &&
         (t.preventDefault(),
         a.getAttribute('data-hover') !== 'true' &&
          (this.toggle(a), (this._topParent = null)));
       }
       if (o.childOf(i, e.parentNode)) return;
       const s = e.classList.contains('menu-toggle') ? e : null;
       s &&
        (t.preventDefault(),
        s.getAttribute('data-hover') !== 'true' && this.toggle(s));
      }
      t.stopPropagation();
     }),
     this._horizontal &&
 ...
 static _findParent(t, n, e = !0) {
  ...
  for (
   t = t.parentNode;
   t.tagName.toUpperCase() !== 'BODY' && !t.classList.contains(n);

  )
   t = t.parentNode;
  if (((t = t.tagName.toUpperCase() !== 'BODY' ? t : null), !t && e))
   throw new Error(`Cannot find \`.${n}\` parent element`);
  return t;
 }
  ...
  for (let s = 0, a = e.length; s < a; s++)
   if (e[s].classList) {
    ...
    for (let r = 0; r < n.length; r++)
     e[s].classList.contains(n[r]) && (l += 1);
 ...
 static _findMenu(t) {
  let n = t.childNodes[0],
   e = null;
  for (; n && !e; )
   n.classList && n.classList.contains('menu-sub') && (e = n),
    (n = n.nextSibling);
  if (!e)
   throw new Error(
 ...
 open(t, n = this._closeChildren) {
  ...
  o._promisify(this._onOpen, this, e, i, o._findMenu(e))
   .then(() => {
    !this._horizontal || !o._isRoot(e)
     ? this._animate && !this._horizontal
       ? (window.requestAnimationFrame(() => this._toggleAnimation(!0, e, !1)),
         this._accordion && this._closeOther(e, n))
       : this._animate
         ? (this._toggleDropdown(!0, e, n),
           this._onOpened && this._onOpened(this, e, i, o._findMenu(e)))
 ...
 close(t, n = this._closeChildren, e = !1) {
  ...
  !i.classList.contains('open') ||
   ...
   o
    ._promisify(this._onClose, this, i, s, o._findMenu(i), e)
    .then(() => {
     if (!this._horizontal || !o._isRoot(i))
      if (this._animate && !this._horizontal)
       window.requestAnimationFrame(() => this._toggleAnimation(!1, i, n));
      else {
 ...
 _toggleDropdown(t, n, e) {
  ...
  let a = !1;
  if (t) {
   o._findParent(n, 'menu-sub', !1) &&
    ((a = !0), (n = this._topParent ? this._topParent.parentNode : n));
   ...
   h - 5 <= -1 * r
    ? (this._innerPosition = -1 * h)
    : h + r + d + 5 >= l &&
      (d > l
       ? (this._innerPosition = -1 * h)
       : (this._innerPosition = -1 * (h + d - l))),
    s.classList.add('open');
   const c = Math.round(i.getBoundingClientRect().width);
   a
    ? h + this._innerPosition + c * 2 > l &&
      c < l &&
      c >= d &&
      (i.style.left = [this._rtl ? '100%' : '-100%'])
    : h + this._innerPosition + c > l &&
      c < l &&
      c > d &&
      (i.style[this._rtl ? 'marginRight' : 'marginLeft'] = `-${c - d}px`),
    this._closeOther(s, e),
    this._updateSlider();
  } else {
   const l = o._findChild(n, ['menu-toggle']);
   if (
    (l.length && l[0].removeAttribute('data-hover', 'true'),
 ...
 _slide(t) {
  ...
  let i;
  t === 'next'
   ? ((i = this._getSlideNextPos()), e + i < n && (i = n - e))
   : ((i = this._getSlidePrevPos()), i > 0 && (i = 0)),
 ...
 static _getItem(t, n) {
  ...
  if (
   (t.classList.contains('menu-item')
    ? o._findChild(t, [i]).length && (e = t)
    : t.classList.contains(i) &&
      (e = t.parentNode.classList.contains('menu-item') ? t.parentNode : null),
   !e)
  )
   throw new Error(`${n ? 'Toggable ' : ''}\`.menu-item\` element not found.`);
  return e;
 }
 static _getLink(t, n) {
  let e = [];
  ...
  if (
   (t.classList.contains(i)
    ? (e = [t])
    : t.classList.contains('menu-item') && (e = o._findChild(t, [i])),
   !e.length)
  )
 ...
 _findUnopenedParent(t, n) {
  ...
  for (; t; )
   t.classList.contains('disabled')
    ? ((i = null), (e = []))
    : (t.classList.contains('open') || (i = t), e.push(t)),
    (t = o._findParent(t, 'menu-item', !1));
  if (!i) return null;
 ...
 _toggleAnimation(t, n, e) {
  ...
  const l = () => {
   n.classList.remove('menu-item-animating'),
    ...
    (n.style.height = null),
    this._horizontal || this.update();
  };
  t
   ? ((n.style.height = `${a}px`),
     n.classList.add('menu-item-animating'),
     n.classList.add('open'),
     o._bindAnimationEndEvent(n, () => {
 ...
 static _bindAnimationEndEvent(t, n) {
  ...
  (i = parseFloat(i) * (i.indexOf('ms') !== -1 ? 1 : 1e3)),
   (t._menuAnimationEndEventCb = e),
   m.forEach((s) => t.addEventListener(s, t._menuAnimationEndEventCb, !1)),
 ...
 _getItemOffset(t) {
  let n = this._inner.childNodes[0],
   e = 0;
  for (; n !== t; )
   n.tagName && (e += Math.round(n.getBoundingClientRect().width)),
    (n = n.nextSibling);
  return e;
 }
 _updateSlider(t = null, n = null, e = null) {
  const i =
    t !== null ? t : Math.round(this._wrapper.getBoundingClientRect().width),
   s = n !== null ? n : this._innerWidth,
   a = e !== null ? e : this._innerPosition;
  s < i || window.innerWidth < window.Helpers.LAYOUT_BREAKPOINT
 ...
 get _innerWidth() {
  ...
  for (let e = 0, i = t.length; e < i; e++)
   t[e].tagName && (n += Math.round(t[e].getBoundingClientRect().width));
  return n;
 }
 get _innerPosition() {
  return parseInt(
 ...
 set _innerPosition(t) {
  return (
   (this._inner.style[this._rtl ? 'marginRight' : 'marginLeft'] = `${t}px`), t
  );
 }
 static _unbindAnimationEndEvent(t) {
  ...
  t._menuAnimationEndEventTimeout &&
   ...
   (t._menuAnimationEndEventTimeout = null)),
   n &&
    (m.forEach((e) => t.removeEventListener(e, n, !1)),
    (t._menuAnimationEndEventCb = null));
 }
 ...
 update() {
  if (!this._horizontal) this._scrollbar && this._scrollbar.update();
   ...
   t - e > n && ((e = t - n), e > 0 && (e = 0), (this._innerPosition = e)),
    this._updateSlider(t, n, e);
  }
 }
 manageScroll() {
  const { PerfectScrollbar: t } = window,
 ...
 switchMenu(t) {
  ...
  if (t === 'vertical') {
   (this._horizontal = !1),
    i.insertBefore(d, r),
    i.insertBefore(h, r),
    i.classList.add('flex-column', 'p-0'),
    s.classList.remove(...s.classList),
    s.classList.add(...l, this._menuBgClass),
    d.classList.remove('d-none', 'd-lg-flex'),
    c.classList.remove('d-none'),
    h.classList.add('overflow-auto');
   for (let u = 0; u < _.length - 1; ++u) _[u].classList.add('open');
  } else {
   (this._horizontal = !0),
    n.children[0].insertBefore(d, e),
    d.classList.add('d-none', 'd-lg-flex'),
    r.appendChild(h),
    i.classList.remove('flex-column', 'p-0'),
    s.classList.remove(...s.classList),
    s.classList.add(...a, this._menuBgClass),
    c.classList.add('d-none'),
    h.classList.remove('overflow-auto');
 ...
 destroy() {
  ...
  for (let e = 0, i = t.length; e < i; e++)
   o._unbindAnimationEndEvent(t[e]),
    t[e].classList.remove('menu-item-animating'),
    t[e].classList.remove('open'),
    (t[e].style.overflow = null),
    (t[e].style.height = null);
  const n = this._el.querySelectorAll('.menu-menu');
  for (let e = 0, i = n.length; e < i; e++)
   (n[e].style.marginRight = null), (n[e].style.marginLeft = null);
