...
(function (a, b) {
 ...
 function cf() {
  try {
   return new a.ActiveXObject('./Microsoft.XMLHTTP.html');
  } catch (b) {}
 }
 function ce() {
  try {
    ...
    e.extend({
     ...
     parseXML: function (b, c, d) {
      a.DOMParser
       ...
       : ((c = new ActiveXObject('./Microsoft.XMLDOM.html')),
         (c.async = 'false'),
         c.loadXML(b)),
       (d = c.documentElement),
  ...
  f.extend({
   ...
   queue: function (a, c, d) {
    if (a) {
     ...
     d &&
      (!e || f.isArray(d) ? (e = f.data(a, c, f.makeArray(d), !0)) : e.push(d));
     return e || [];
    }
   },
    ...
    if (!c.isDefaultPrevented()) {
     ...
     if (
      ...
      try {
       l &&
        e[h] &&
        ((n = e[l]), n && (e[l] = null), (f.event.triggered = h), e[h]());
      } catch (p) {}
  ...
  handle: function (c) {
   ...
   for (var h = 0, i = d.length; h < i; h++) {
    ...
    if (e || c.namespace_re.test(j.namespace)) {
     ...
     k !== b &&
      ((c.result = k), k === !1 && (c.preventDefault(), c.stopPropagation()));
   ...
   (f.event.special.submit = {
    setup: function (a, b) {
     if (!f.nodeName(this, 'form'))
      f.event.add(this, './click.specialSubmit.html', function (a) {
       var b = a.target,
        c = b.type;
       (c === 'submit' || c === 'image') &&
       ...
       f.event.add(this, './keypress.specialSubmit.html', function (a) {
        var b = a.target,
         c = b.type;
        (c === 'text' || c === 'password') &&
         f(b).closest('form').length &&
   ...
   var l = (k.selectors = {
     ...
     filter: {
      PSEUDO: function (a, b, c, d) {
       ...
       if (e === 'contains')
        return (
         (a.textContent || a.innerText || k.getText([a]) || '').indexOf(b[3]) >=
         0
        );
       if (e === 'not') {
        var g = b[3];
     ...
     (a.innerHTML = "<a name='" + d + "'/>"),
      ...
      c.getElementById(d) &&
       ...
       (l.filter.ID = function (a, b) {
        var c =
         typeof a.getAttributeNode != 'undefined' && a.getAttributeNode('id');
