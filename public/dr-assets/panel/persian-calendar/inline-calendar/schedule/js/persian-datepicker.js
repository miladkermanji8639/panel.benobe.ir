...
(function (c, j) {
 ...
 function l(a) {
  return !c(a)
   ...
   .filter(function () {
    return (
     c.curCSS(this, 'visibility') === 'hidden' || c.expr.filters.hidden(this)
    );
   }).length;
 }
 c.ui = c.ui || {};
 ...
 $.extend(Datepicker.prototype, {
  ...
  _generateHTML: function (inst) {
   ...
   if (maxDate) {
    ...
    maxDraw =
     minDate && this._compareDate(maxDraw, '<', minDate) ? minDate : maxDraw;
    while (
  ...
  _generateMonthYearHeader: function (
   ...
   var html =
    '<div class="ui-datepicker-title d-flex justify-content-center align-items-center">'; // وسط‌چین کردن
   var monthHtml = '';
   var yearHtml = '';
   // تبدیل سال میلادی به شمسی
   var convertToJalaliYear = function (year) {
    var gregorianDate = new Date(year, 0, 1);
    var jalaliDate = new JalaliDate(gregorianDate);
    return jalaliDate.getFullYear();
   };
   // ایجاد سلکت آپشن برای ماه
   if (secondary || !changeMonth) {
    ...
    monthHtml +=
     '<select class="ui-datepicker-month form-control form-control-sm col-6" onchange="DP_jQuery_' +
     dpuuid +
   ...
   if (!inst.yearshtml) {
    ...
    if (secondary || !changeYear) {
     ...
     inst.yearshtml +=
      '<select class="ui-datepicker-year form-control form-control-sm col-6" onchange="DP_jQuery_' +
      dpuuid +
      ".datepicker._selectMonthYear('#" +
      inst.id +
     ...
     for (; year <= endYear; year++) {
      var jalaliYear = convertToJalaliYear(year);
      inst.yearshtml +=
       '<option value="' +
       year +
       '"' +
   ...
   // تنظیم ترتیب نمایش ماه و سال بر اساس showMonthAfterYear
   if (showMonthAfterYear) {
    html += yearHtml + '&#xa0;' + monthHtml;
   } else {
    html += monthHtml + '&#xa0;' + yearHtml;
   }
   html += '</div>'; // پایان دادن به div
   return html;
  },
  ...
  _getDaysInMonth: function (year, month) {
   return (
    32 - this._daylightSavingAdjust(new this.CDate(year, month, 32)).getDate()
   );
  },
 ...
 a.datepicker.regional.ar = {
  ...
  calculateWeek: function (b) {
   ...
   return (
    Math.floor(
     Math.round(
      (c.getTime() - new HijriDate(c.getFullYear(), 0, 1).getTime()) / 86400000,
     ) / 7,
    ) + 1
   );
  },
