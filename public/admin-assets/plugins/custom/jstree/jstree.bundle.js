 ...
 $.jstree.core.prototype = {
  ...
  init: function (el, options) {
   ...
   this._data.core.original_container_html
    ...
    .filter(function () {
     return (
      this.nodeType === 3 && (!this.nodeValue || /^\s+$/.test(this.nodeValue))
     );
    })
    .remove();
   this.element.html(
  ...
  redraw_node: function (node, deep, is_callback, force_render) {
   ...
   if (obj.state.loaded && !has_children) {
    ...
    c +=
     obj.state.opened && obj.state.loaded ? ' jstree-open' : ' jstree-closed';
    node.childNodes[1].setAttribute(
     'aria-expanded',
  ...
  move_node: function (
   ...
   if (is_multi) {
    if (
     (tmp = this.copy_node(obj, par, pos, callback, is_loaded, false, origin))
    ) {
     if (old_ins) {
