  ...
  init: function () {
   (t = document.querySelector('#kt_modal_adjust_balance')),
    ...
    (function () {
     ...
     let o,
      a = parseFloat(e.innerHTML.replace(/[^0-9.]/g, '').replace(',', ''));
     (a = n ? -1 * a : a),
      i.addEventListener('focusout', function (t) {
       (o = parseFloat(
        ...
        isNaN(o) && (o = 0),
        (r.innerHTML =
         'US$ ' + (o + a).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
      });
    })(),
    (a = FormValidation.formValidation(l, {
     fields: {
      adjustment: {
       validators: { notEmpty: { message: 'Adjustment type is required' } },
      },
      amount: { validators: { notEmpty: { message: 'مبلغ مورد نیاز است' } } },
     },
     plugins: {
            ...
            setTimeout(function () {
             e.removeAttribute('data-kt-indicator'),
              Swal.fire({
               ...
               t.isConfirmed &&
                (c.hide(), (e.disabled = !1), l.reset(), (r.innerHTML = '--'));
              });
            }, 2e3))
          : Swal.fire({
             text:
