...
function parseDate(string, convertToPersian) {
 ...
 var separator = match[2],
  ...
  tzOffset =
   (match[12] == '-' ? -1 : 1) * ((+match[13] || 0) * 60 + (+match[14] || 0));
 ...
 if (isNonLocal) {
  date.setUTCMinutes(
   date.getUTCMinutes() - date.getTimezoneOffset() + tzOffset,
  );
 }
 return date;
}
...
function JDate(a, month, day, hour, minute, second, millisecond) {
 ...
 } else if (arguments.length == 0) this._d = new Date();
 else if (arguments.length == 1) {
  this._d = new Date(a instanceof JDate ? a._d : a);
 } else {
  var persian = jd_to_gregorian(
   persian_to_jd_fixed(a, (month || 0) + 1, day || 1),
  );
  this._d = new Date(
   persian[0],
   persian[1] - 1,
...
JDate.prototype = {
 ...
 _setPersianDate: function (which, value, dayValue) {
  ...
  var new_date = jd_to_gregorian(
   persian_to_jd_fixed(persian[0], persian[1], persian[2]),
  );
...
var i,
 dateProps = (
  'getHours getMilliseconds getMinutes getSeconds getTime getUTCDay getUTCHours ' +
