...
function incrementTime(
 type: 'currentYearElement' | 'hourElement' | 'minuteElement' | 'secondElement',
 by: number,
) {
 const e = fp[type];
...
describe('flatpickr', () => {
 ...
 describe('API', () => {
  ...
  it('parses dates in enable[] and disable[]', () => {
   ...
   expect((fp.config.disable[0] as DateRangeLimit).from instanceof Date).toBe(
    true,
   );
 ...
 describe('UI', () => {
  ...
  it('static calendar', () => {
   ...
   expect(
    (fp.element.parentNode as Element).classList.contains('flatpickr-wrapper'),
   ).toBe(true);
  ...
  it('year input', () => {
   ...
   expect((fp.days.childNodes[10] as DayElement).dateObj.getFullYear()).toEqual(
    2001,
   );
  });
  ...
  it('should have working strap mode', () => {
   ...
   instance.selectedDateElem &&
    expect(parseInt(instance.selectedDateElem.textContent as string)).toEqual(
     new Date().getDate(),
    );
