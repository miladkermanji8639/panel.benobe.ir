...
function FlatpickrInstance(
  ...
  if (self.config.wrap) {
   ['open', 'close', 'toggle', 'clear'].forEach((evt) => {
    Array.prototype.forEach.call(
     ...
     (el: HTMLElement) =>
      bind(el, 'click', self[evt as 'open' | 'close' | 'toggle' | 'clear']),
    );
   });
  }
 ...
 function clear(triggerChangeEvent = true) {
  ...
  if (self.config.enableTime) {
   if (self.config.minDate !== undefined) setHoursFromDate(self.config.minDate);
   else
    setHours(
     self.config.defaultHour,
     self.config.defaultMinute,
 ...
 function changeYear(newYear: number) {
  if (
   ...
   (self.currentYearElement.getAttribute('data-min') &&
    newYear <
     parseInt(self.currentYearElement.getAttribute('data-min') as string)) ||
   ...
   switch (e.keyCode) {
    ...
    case 40:
     ...
     if (self.daysContainer && (e.target as DayElement).$i !== undefined) {
      ...
      } else if (!isTimeObj) focusOnDay((e.target as DayElement).$i, delta * 7);
 ...
 function onMouseOver(elem: DayElement) {
  ...
  for (
   ...
   i++,
    date =
     self.days.childNodes[i] && (self.days.childNodes[i] as DayElement).dateObj
  ) {
  ...
  for (let i = 0; i < self.config.plugins.length; i++) {
   ...
   for (const key in pluginConf) {
    if (~hooks.indexOf(key as HookKey)) {
     self.config[key as keyof Options] = arrayify(
 ...
 function selectDate(e: MouseEvent | KeyboardEvent) {
  ...
  if (self.config.enableTime) setTimeout(() => (self.showTimeInput = true), 50);
  ...
  if (self.config.mode === 'range') {
   if (self.selectedDates.length === 1) {
    ...
    self._hidePrevMonthArrow =
     self._hidePrevMonthArrow ||
     (self.minRangeDate !== undefined &&
      self.minRangeDate > (self.days.childNodes[0] as DayElement).dateObj);
 ...
 function timeWrapper(e: MouseEvent | KeyboardEvent | IncrementEvent): void {
  ...
  const min = parseFloat(input.getAttribute('data-min') as string),
   ...
   delta =
    (e as IncrementEvent).delta || (isKeyDown ? (e.which === 38 ? 1 : -1) : 0);
