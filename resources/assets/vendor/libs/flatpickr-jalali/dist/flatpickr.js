...
***************************************************************************** */
 ...
 var revFormat = {
  ...
  U: function (_, unixSeconds) {
   return new JDate(parseFloat(unixSeconds) * 1000);
  },
 ...
 var createDateParser = function (_a) {
  ...
  return function (date, givenFormat, timeless) {
   ...
   var parsedDate;
   var date_orig = date;
   if (date instanceof Date) parsedDate = new JDate(date.getTime());
 ...
 function compareDates(date1, date2, timeless) {
  ...
  if (timeless !== false) {
   return (
    new JDate(date1.getTime()).setHours(0, 0, 0, 0) -
 ...
 function FlatpickrInstance(element, instanceConfig) {
    ...
    getDaysInMonth: function (month, yr) {
     ...
     if (self.config.locale === 'fa') {
      ...
      if (calcedMonth < 6) {
       ...
       ((((calcedYear - (calcedYear > 0 ? 474 : 473)) % 2820) + 474 + 38) *
        682) %
        2816 <
       682
      ) {
    ...
    if (isDateSelected(date)) {
     ...
     if (self.config.mode === 'range') {
      ...
      toggleClass(
       dayElement,
       'endRange',
       self.selectedDates[1] && compareDates(date, self.selectedDates[1]) === 0,
      );
     }
    }
   } else {
  ...
  function buildDays() {
   ...
   var firstOfMonth =
     (new JDate(self.currentYear, self.currentMonth, 1).getDay() -
   ...
   if (isRangeMode) {
    self.minRangeDate = new JDate(
     self.currentYear,
     self.currentMonth - 1,
     dayNumber,
    );
    self.maxRangeDate = new JDate(
     self.currentYear,
     self.currentMonth + 1,
   ...
   for (; dayNumber <= prevMonthDays; dayNumber++, dayIndex++) {
    days.appendChild(
     createDay(
      'prevMonthDay',
      new JDate(self.currentYear, self.currentMonth - 1, dayNumber),
      dayNumber,
      dayIndex,
     ),
    );
   ...
   for (dayNumber = 1; dayNumber <= daysInMonth; dayNumber++, dayIndex++) {
    days.appendChild(
     createDay(
      '',
      new JDate(self.currentYear, self.currentMonth, dayNumber),
      dayNumber,
      dayIndex,
     ),
    );
   ...
   for (
    ...
    days.appendChild(
     createDay(
      'nextMonthDay',
      new JDate(self.currentYear, self.currentMonth + 1, dayNum % daysInMonth),
      dayNum,
      dayIndex,
     ),
    );
  ...
  function changeYear(newYear) {
   if (
    ...
    (self.currentYearElement.getAttribute('data-min') &&
     newYear < parseInt(self.currentYearElement.getAttribute('data-min'))) ||
     ...
     case 39:
      if (!isTimeObj) {
       ...
       if (self.daysContainer) {
        ...
        else changeMonth(delta_1, true, true);
       }
  ...
  function selectDate(e) {
   ...
   var target = t;
   var selectedDate = (self.latestSelectedDateObj = new JDate(
  ...
  function timeWrapper(e) {
   ...
   if (typeof input.value !== 'undefined' && input.value.length === 2) {
    ...
    if (newValue < min) {
     newValue =
      max + newValue + int(!isHourElem) + (int(isHourElem) && int(!self.amPM));
