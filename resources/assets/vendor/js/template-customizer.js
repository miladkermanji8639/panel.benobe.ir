...
class TemplateCustomizer {
 constructor({
  ...
  this.settings.availableLayouts =
   availableLayouts || TemplateCustomizer.LAYOUTS;
 ...
 _loadSettings() {
  ...
  if (
   lType !== '' &&
   ['static', 'static-offcanvas', 'fixed', 'fixed-offcanvas'].indexOf(lType) !==
    -1
  ) {
   type = lType;
  } else {
  ...
  this.settings.contentLayout =
   contentLayout !== '' ? contentLayout : this.settings.defaultContentLayout;
  ...
  if (
   navbarOption !== '' &&
   ['static', 'sticky', 'hidden'].indexOf(navbarOption) !== -1
  ) {
 ...
 _setup(_container = document) {
  ...
  if (!this._hasControls('style')) {
   ...
   this.settings.availableStyles.forEach((style) => {
    const styleEl = createOptionElement(
     style.name,
     style.title,
     'customRadioIcon',
